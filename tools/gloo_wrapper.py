...
def allgather_sizes(send_data, world_size):
    ...
    Parameters
    ----------
    send_data : Data on which allgather is performed.
    world_size : No. of processes configured for execution
    """

    #compute the length of the local data
    send_length = len(send_data)
    out_tensor = torch.as_tensor(send_data, dtype=torch.int32)
    in_tensor = [torch.zeros(send_length, dtype=torch.int32) 
...
def gather_metadata_json(metadata, rank, world_size): 
    """
    Gather an object (json schema on rank-0)

    Parameters:
    -----------
    metadata : json schema formed on each rank with graph level data. 
