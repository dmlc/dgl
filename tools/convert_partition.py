...
def processMetisPartitions (isCmdLineExec, pipelineArgs, args): 
    ...
    #Read the json file for the entire graph
    with open(input_dir+'/'+schema_file) as json_file:
    ...
    for part_id in lst_num_parts:
        ...
        for ntype_name in nid_ranges: 
            ...
            node_map_val[ntype_name].append([int(node_id_start),
                                        int(node_id_start + np.sum(ntype_ids == ntype_id))])
            node_id_start += np.sum(ntype_ids == ntype_id)
        ...
        if node_attr_dtype is not None:
            ...
            if isCmdLineExec == True: 
                ...
                # This case is for distributed pipeline processing. 
    ...
    '''
    End of the for loop -- over partitions
    '''
