...
def read_metis_partitions( metis_mappings ):
    """
    ...
    Parameters:
    -----------
    metis_mappings : file name which is the output of metis partitioning
                     algorithm (pm_dglpart2, in the METIS installation.
    """

    metis_map = np.loadtxt( metis_mappings, delimiter=' ', dtype='int')
...
def read_json(json_file):
    ...
    Parameters:
    -----------
    json_file : file name for the json schema
    """
...
def get_node_types(schema):
    ...
    Parameters:
    -----------
    schema : Input schema from which the node_typename -> node_type
...
def write_metadata_json(metadata_list, output_dir, graph_name):
    ...
    Parameters:
    -----------
    metadata_list : list of json schemas to merge
                  : no. of json schemas should be world_size
    output_dir : output directory path in which results are stored as files
    graph-name : a string specifying the graph name
    """
...
def augment_edge_data(edge_data, partitions):
    """
    Add tgt rank column to the edge_data for convenience
    
    Parameters:
    -----------
    edge_data : Edge information as read from the xxx_edges.txt file
    partitions : METIS assigned partitions for a graph
    """
...
def augment_node_data(node_data, partitions): 
    ...
    Parameters:
    -----------
    node_data : Node information as read from xxx_nodes.txt file
    partitions : METIS assigned partitions for a graph
    """
    global_ids = np.arange(node_data.shape[0])
    node_data_aug = np.c_[node_data, global_ids]
...
def read_nodes_file(nodes_file):
    ...
    Parameters:
    -----------
    nodesfile : Graph file for nodes in the input graph
    ...
    #
    nodes_data = np.loadtxt( nodes_file, delimiter=' ', dtype='int')
    return nodes_data
...
def read_edges_file( edge_file ):
    ...
    Parameters:
    -----------
    edge_file : Graph file for edges in the input graph
...
def read_node_features_file (nodes_features_file):
    ...
    Parameters:
    -----------
    nodes_features_file : Features file for nodes in the graph
    """
...
def read_edge_features_file( edge_features_file ):
    ...
    Parameters:
    -----------
    edge_features_file : Features file for edges in the graph
    """
    edge_features = dgl.data.utils.load_tensors( edge_features_file, True )
    return edge_features


def writeNodeFeatures( node_features, node_file ):
    """
    Utility function to node_features file

    Parameters:
    -----------
    node_features : file name in which node features are stored
    node_file     : File in which the node' finrmation)
    """
