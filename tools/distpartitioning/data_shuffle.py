...
def exchange_edge_data(rank, world_size, num_parts, edge_data, id_lookup):
    ...
    all_counts = allgather_sizes([num_edges], world_size, num_parts, return_sizes=True)
    max_edges = np.amax(all_counts)
    all_edges = np.sum(all_counts)
    num_chunks = (max_edges // CHUNK_SIZE) + (0 if (max_edges % CHUNK_SIZE == 0) else 1)
    LOCAL_CHUNK_SIZE = (num_edges // num_chunks) + (0 if (num_edges % num_chunks == 0) else 1)
    logging.info(f'[Rank: {rank} Edge Data Shuffle - max_edges: {max_edges}, \
    ...
    for local_part_id in range(num_parts//world_size):
        ...
        for chunk in range(num_chunks):
            ...
            logging.info(f'[Rank: {rank}] EdgeData Shuffle: processing \
            ...
            for idx in range(world_size):
                ...
                filt_data = np.column_stack((cur_src_id[send_idx == 1], \
        ...
        edge_data[constants.GLOBAL_SRC_ID+"/"+str(local_part_id)] = np.concatenate(local_src_ids)
        edge_data[constants.GLOBAL_DST_ID+"/"+str(local_part_id)] = np.concatenate(local_dst_ids)
        edge_data[constants.GLOBAL_TYPE_EID+"/"+str(local_part_id)] = np.concatenate(local_type_eids)
        edge_data[constants.ETYPE_ID+"/"+str(local_part_id)] = np.concatenate(local_etype_ids)
        edge_data[constants.GLOBAL_EID+"/"+str(local_part_id)] = np.concatenate(local_eids)

    # Check if the data was exchanged correctly
    local_edge_count = 0
    for local_part_id in range(num_parts//world_size):
        local_edge_count += edge_data[constants.GLOBAL_SRC_ID+"/"+str(local_part_id)].shape[0]
    shuffle_edge_counts = allgather_sizes([local_edge_count], world_size, num_parts, return_sizes=True)
