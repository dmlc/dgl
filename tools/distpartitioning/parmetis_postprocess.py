...
def post_process(params):
    ...
    logging.info(params.schema)
    logging.info(params.parmetis_output)
    assert os.path.isfile(params.schema)
    assert os.path.isfile(params.parmetis_output)
    schema = read_json(params.schema)

    metis_df = csv.read_csv(params.parmetis_output, 
    ...
    type_nid_dict, ntype_gnid_offset = \
        get_idranges(schema[constants.STR_NODE_TYPE],
    ...
    for ntype_id, ntype_name in ntid_ntype_map.items():
        ...
        csv.write_csv(pyarrow.Table.from_arrays([out_data], 
            names=["partition-ids"]), out_file, options)
...
if __name__ == "__main__":
    ...
    """
    parser = argparse.ArgumentParser(description='PostProcessing the ParMETIS\
        output for partitioning pipeline')
    parser.add_argument('--schema', required=True, type=str,
                     help='The schema of the input graph')
    parser.add_argument('--parmetis_output', required=True, type=str,
        help='ParMETIS output file')
    parser.add_argument('--output_dir', required=True, type=str, help='The output\
