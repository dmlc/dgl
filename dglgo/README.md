...
## Installation guide
You can install DGL-enter easily by `pip install dglcli`. Then you should be able to use DGL-Enter in you commandline tool by type in `dgl-enter`
```
...
Run `dgl-enter config nodepred --data cora --model sage --cfg cora_sage.yaml`. Then you'll get a configuration file `cora_sage.yaml` includes all the configuration to be tuned, with the comments
...
Simply run `dgl-enter train --cfg cora_sage.yaml` will start the training process
```log
...
...
Try `dgl-enter export --cfg cora_sage.yaml --output script.py`, and you'll get the script used to train the model, like a magic!
...
For example, using GCN with pubmet dataset, you can use `enter/recipes/nodepred_pubmed_gcn.yaml`. 

To try it, type in `dgl-enter train --cfg recipes/nodepred_pubmed_gcn.yaml` to train the model, or `dgl-enter export --cfg recipes/nodepred_pubmed_gcn.yaml` to get the full training script.
...
Following the tutorial at [Loading data from CSV files](https://docs.dgl.ai/en/latest/guide/data-loadcsv.html#guide-data-pipeline-loadcsv`), Prepare your own CSV dataset includes three files minimally, node data csv, edge data csv and the meta data file (meta.yaml).
...
Try `dgl-enter config nodepred --data csv --model sage --cfg csv_sage.yaml`, to use SAGE model for your dataset. You'll see the data part is now the configuration related to CSV dataset. `data_path` is used to specify the data folder, and `./` means the current folder. 

If your dataset doesn't have the builtin split on the nodes for train/val/test, you need to manually set the split ratio in the config yaml file, DGL will random generate the split for you.
...
Then you can do the same as the tutorial above, either train the model by `dgl-eneter train --cfg csv_sage.yaml` or use `dgl-enter export --cfg csv_sage.yaml --output my_dataset.py` to get the training script.
...
Options:
  --cfg TEXT  yaml file name  [default: cfg.yaml]
  --help      Show this message and exit.
```
