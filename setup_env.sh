...
echo "Install DGL..."
( git clone --recursive https://github.com/sanchit-misra/dgl.git -b xeon-optimizations && cd dgl && git checkout b8aa727 && rm -rf build && mkdir build && cd build && cmake ../ &&  make -j && cd ../python && python setup.py clean && CMAKE_C_COMPILER=gcc CMAKE_CXX_COMPILER=g++ python setup.py install ) 
