...
#         batch_accuracy = MF.accuracy(out, y, num_classes=num_classes, task="multiclass")
...
#         batch_accuracy = MF.accuracy(out, labels, num_classes=num_classes, task="multiclass")
...
#     parser.add_argument("--dataset", type=str, default="ogbn-arxiv", help="Dataset name (e.g., 'ogbn-arxiv')")
...
#                                    sizes=[10, 10, 10], batch_size=1024, shuffle=True,
#                                    num_workers=12)
...
#     # print(f"Before training - GPU Allocated Memory: {torch.cuda.memory_allocated(device)} bytes")


#     for epoch in range(10):
#         loss, acc = train(model, data, train_loader, optimizer, criterion, device, num_classes)
              
#         val_acc = evaluate(model, data, subgraph_loader, device, num_classes)
#         print(f'Epoch: {epoch}, Loss: {loss:.4f}, Accuracy: {acc:.4f}, Val Acc: {val_acc:.4f}')
...
#     # print(f"After training - Allocated Memory: {torch.cuda.memory_allocated(device)} bytes")
#     # print(f"Peak GPU Memory Usage: {torch.cuda.max_memory_allocated(device)} bytes")
...
def main():
    ...
    train_loader = NeighborSampler(data.edge_index, node_idx=split_idx['train'],
                                   sizes=[10, 10, 10], batch_size=1024, shuffle=True,num_workers=12)
    ...
    subgraph_loader = NeighborSampler(data.edge_index, node_idx=None, sizes=[-1], num_workers=12, batch_size=1024, shuffle=False)
