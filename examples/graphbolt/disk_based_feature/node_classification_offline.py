...
def create_sampler(graph, itemset, batch_size, fanout, device, job):
...
def create_trainloader(features, itemset, device, job):
    assert job == "train"
    # Initialize an ItemSampler to sample mini-batches from the dataset.
    datapipe = gb.ItemSampler(
        itemset,
    ...
    datapipe = datapipe.load_minibatch(subgraph_dir=args.subgraph_dir)
    # Copy the data to the specified device.
...
def main():
    ...
    if args.cpu_cache_size_in_gigabytes > 0 and isinstance(
        ...
        features[("node", None, "feat")] = gb.CPUCachedFeature(
            ...
            int(args.cpu_cache_size_in_gigabytes * 1000 * 1000 * 1000),
    ...
    if args.gpu_cache_size_in_gigabytes > 0 and args.feature_device != "cuda":
        features[("node", None, "feat")] = gb.GPUCachedFeature(
            features[("node", None, "feat")],
            int(args.gpu_cache_size_in_gigabytes * 1000 * 1000 * 1000),
        )
    ...
    num_minibatch = (len(train_set) + args.batch_size - 1) // args.batch_size
    ...
    train_dataloader = create_trainloader(
        features=features,
        itemset=gb.ItemSet(num_minibatch, "seeds"),
        device=args.device,
        job="train",
    )
    ...
    perpare_time = time.time() - start
    print(f"Prepare time: {perpare_time:.2f}s")

    start = time.time()
    if not os.path.exists(args.subgraph_dir):
        os.makedirs(args.subgraph_dir)
        for it, minibatch in enumerate(tqdm(train_sampler, "Sampling")):
            data = [
                minibatch.seeds.cpu(),
            ...
            torch.save(data, f"{args.subgraph_dir}/train-{it}.pt")
    print(f"Sampling time: {time.time() - start:.2f}s")
...
if __name__ == "__main__":
    args = parse_args()
    args.subgraph_dir = os.path.join(
        args.root,
        f"{args.dataset}-{args.batch_size}-{args.fanout}",
    )
    print(args)
    main()
