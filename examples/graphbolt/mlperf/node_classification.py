"""
  ...
  All rights reserved.
This script utilizes GraphBolt data loader for the MLPerf node classification
task: https://mlcommons.org/2024/06/gnn-for-mlperf-training-v4/.
"""
...
class EntityClassify(nn.Module):
    def __init__(
        self, graph, in_size, hidden_size, out_size, n_layers, cooperative
    ):
...
def evaluate_step(minibatch, model):
    ...
    out = model(minibatch, node_features)[category]
...
def train_helper(
    ...
    total_samples = 0
    start = time.time()
    dataloader = tqdm(dataloader, "Training") if rank == 0 else dataloader
    for step, minibatch in enumerate(dataloader):
    ...
    train_loss = weighted_reduce(total_loss, total_samples)
...
def parse_args():
    parser = argparse.ArgumentParser(description="GraphBolt MLPerf submission.")
    parser.add_argument(
        "--epochs", type=int, default=999999, help="Number of training epochs."
    )
    parser.add_argument(
        "--lr",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Dataset name. Default is `igb-het-mlperf`.",
    )
    parser.add_argument(
        "--fanout",
        type=str,
