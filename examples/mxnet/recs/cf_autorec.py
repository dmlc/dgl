...
def gcn_msg(edge):
    ...
    msg_wgt = (edge.data['r']+1) * mask
    return {
        'W'    : edge.src['W'] * (msg_wgt).reshape((-1,1)),
        'mask' : mask
    }
...
def gcn_reduce(node):
    ...
    degree = mx.nd.sum(node.mailbox['mask'], axis=1) + 1e-10
    return {'accum' : accum / degree.sqrt().reshape((-1,1))}
...
def quad_fcn(edge):
    ...
    item_b = mx.nd.where(user2item, edge.dst['b'], edge.src['b']) # edges
    user_h = mx.nd.where(user2item, edge.src['h'], edge.dst['h']) # edges, hidden
    item_W = mx.nd.where(user2item, edge.dst['W'], edge.src['W']) # edges, hidden

    score = item_b + (item_W * user_h).sum(axis=1) # edges

    return {'score' : score}


class CF_AUTOREC(gluon.Block):
    def __init__(self,
                 in_feats,
                 n_hidden,
                 n_classes,
        ...
        super(CF_AUTOREC, self).__init__()
        self.n_hidden   = n_hidden
        self.n_classes  = n_classes
        self.dropout    = dropout

        with self.name_scope():
            self.W = self.params.get('W', shape=(in_feats, n_hidden))
            self.b = self.params.get('b', shape=(in_feats))
    ...
    def forward(self, g, is_data=None):
        ...
        g.ndata['h'] = self.c.data() # user embedding
...
def train(model, l2loss, args):
    ...
    for epoch in range(args.n_epochs):
        ...
        with mx.autograd.record():
            score = model(G, is_data)
            loss = l2loss(score, G.edata['r'], G.edata['is_train'] * is_label)
        ...
        val_loss = l2loss(score, G.edata['r'], G.edata['is_val'])
...
if __name__ == '__main__':
    ...
    model = CF_AUTOREC(len(G), args.n_hidden, n_classes, args.n_layers,
