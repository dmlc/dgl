...
class NodeUpdateModule(gluon.Block):
    def __init__(self, out_feats, activation=None, dropout=0, normalization=None):
    ...
    def forward(self, node):
        if self.normalization == 'sym':
            h = node.data['h'] * node.data['out_degree'].sqrt()
        else:
            h = node.data['h']
        if self.normalization == 'sym':
            accum = node.data['accum'] / node.data['in_degree'].sqrt()
        elif self.normalization == 'left':
            accum = node.data['accum'] / node.data['in_degree']
        else:
            accum = node.data['accum']
        accum = self.linear(accum)
        if self.dropout:
            accum = mx.nd.Dropout(accum, p=self.dropout)
        return {'h': mx.nd.concat(h, accum, dim=1)}
...
class GCN(gluon.Block):
    def __init__(self,
        ...
        for i in range(n_layers):
            self.conv_layers.add(NodeUpdateModule(n_hidden, activation, dropout, normalization))
    ...
    def forward(self, features):
        ...
        for layer in self.conv_layers:
            if self.normalization == 'sym':
                self.g.ndata['h'] = self.g.ndata['h'] / self.g.ndata['out_degree'].sqrt()
...
def main(args):
    ...
    g.ndata['in_degree']  = in_degree.reshape((-1,1))
    g.ndata['out_degree'] = out_degree.reshape((-1,1))
