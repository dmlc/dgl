...
class NodeUpdate(gluon.Block):
    ...
    def forward(self, node):
        ...
        if self.test:
            ...
            # control variate
            h = (h - node.data['self_delta_h']) * node.data['subg_norm'] + agg_history * norm
            h = mx.nd.concat(h, self_h)
            if self.dropout:
...
class GCNSampling(gluon.Block):
    def __init__(self,
        ...
        with self.name_scope():
            ...
            for i in range(1, n_layers):
                self.layers.add(NodeUpdate(i, 2*n_hidden, n_hidden, n_hidden, dropout, activation))
            # output layer
            self.layers.add(NodeUpdate(n_layers, 2*n_hidden, n_classes, n_hidden, dropout))
    ...
    def forward(self, nf):
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_nid = map_to_nodeflow_nid(nf, i, parent_nid).tousertensor()
            self_h = h[layer_nid]
...
class GCNInfer(gluon.Block):
    def __init__(self,
        ...
        with self.name_scope():
            ...
            # hidden layers
            for i in range(1, n_layers):
                self.layers.add(NodeUpdate(i, 2*n_hidden, n_hidden, n_hidden, 0, activation, True))
            # output layer
            self.layers.add(NodeUpdate(n_layers, 2*n_hidden, n_classes, n_hidden, 0, None, True))
