...
#     indices = tf.argsort(score, axis=1, direction='DESCENDING')
#     indices = torch.nonzero(indices == tf.reshape(target, (-1, 1)))
...
#         ranks = tf.concat([ranks_s, ranks_o])
#         ranks += 1 # change to 1-indexed

#         mrr = tf.mean(1.0 / ranks.float())
#         print("MRR (raw): {:.6f}".format(mrr.item()))

#         for hit in hits:
#             avg_count = tf.mean(tf.cast(ranks <= hit, tf.float32))
