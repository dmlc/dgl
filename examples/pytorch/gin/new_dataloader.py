...
def train(train_loader, val_loader, device, model):
    ...
    for epoch in range(350):
        ...
        print("Epoch {:05d} | Loss {:.4f} | Train Acc. {:.4f} | Validation Acc. {:.4f} | Time {:.5f}"
              . format(epoch, total_loss / (batch + 1), train_acc, valid_acc, time.time() - start))
...
if __name__ == '__main__':
    ...
    graph_source = DatasetGraphSource(dataset)

    # create dataloader
    train_loader = DataLoader2(graph_source=graph_source, feature_source=None,
        ids=train_idx, batch_size=128, shuffle=True, output_device=device)
    val_loader = DataLoader2(graph_source=graph_source, feature_source=None,
        ids=val_idx, batch_size=128, shuffle=True, output_device=device)
