...
def train(optimizer,loss_fn,model,prep,dataloader,lambda_reg,device):
    total_loss = 0
    for i,(graph_batch,data_batch,label_batch) in enumerate(dataloader):
        ...
        v_pred,out_e = model(graph_batch,node_feat[:,3:5].float(),edge_feat.float(),dummy_global,dummy_relation)
        loss = loss_fn(v_pred,label_batch)
        total_loss += float(loss)
        zero_target = torch.zeros_like(out_e)
        loss = loss + lambda_reg*loss_fn(out_e,zero_target)*zero_target.numel()/2
        reg_loss = 0
        for param in model.parameters():
            reg_loss = reg_loss + lambda_reg*loss_fn(param,torch.zeros_like(param).float().to(device)*param.numel()/2)
        loss = loss + reg_loss
        loss.backward()
        optimizer.step()
...
def eval(loss_fn,model,prep,dataloader,device):
    ...
    with torch.no_grad():
        for i,(graph_batch,data_batch,label_batch) in enumerate(dataloader):
            ...
            dummy_relation = torch.zeros(edge_feat.shape[0],1).float().to(device)
            dummy_global   = torch.zeros(node_feat.shape[0],1).float().to(device)
            v_pred,_ = model(graph_batch,node_feat[:,3:5].float(),edge_feat.float(),dummy_global,dummy_relation)
            loss = loss_fn(v_pred,label_batch)
            total_loss += float(loss)
    return total_loss/(i+1)
...
def eval_rollout(model,prep,initial_frame,n_object,device):
    ...
    with torch.no_grad():
        for step in range(100):
            ...
            v_pred,_ = model(graph,node_feats[:,3:5].float(),edge_feats.float(),dummy_global,dummy_relation)
            current_frame[:,[1,2]] += v_pred*0.001
            current_frame[:,3:5] = v_pred
            pos_buffer.append(current_frame[:,[1,2]].cpu().numpy()) # Should be [1000,6,2]
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(train_data,args.batch_size,True,collate_fn=collator,num_workers=args.num_workers)
    valid_dataloader = DataLoader(valid_data,args.batch_size,True,collate_fn=collator,num_workers=args.num_workers)
    test_full_dataloader = DataLoader(test_data,args.batch_size,True,collate_fn=collator,num_workers=args.num_workers)
    ...
    print("Weight: ",train_data.stat_median[0],train_data.stat_max[0],train_data.stat_min[0])
    print("Position: ",train_data.stat_median[[1,2]],train_data.stat_max[[1,2]],train_data.stat_min[[1,2]])
    print("Velocity: ",train_data.stat_median[[3,4]],train_data.stat_max[[3,4]],train_data.stat_min[[3,4]])
    ...
    try:
        for e in range(args.epochs):
            last_t = time.time()
            loss = train(optimizer,loss_fn,interaction_net,prepare_layer,train_dataloader,args.lambda_reg,device)
            print("Epoch time: ",time.time()-last_t)
            if e %1 ==0:
                valid_loss = eval(loss_fn,interaction_net,prepare_layer,valid_dataloader,device)
                test_full_loss = eval(loss_fn,interaction_net,prepare_layer,test_full_dataloader,device)
                print("Epoch: {}.Loss: Valid: {} Full: {}".format(e,valid_loss,test_full_loss))
    except:
        traceback.print_exc()
    finally:
        if args.visualize:
            eval_rollout(interaction_net,prepare_layer,test_data.first_frame,test_data.n_particles,device)
