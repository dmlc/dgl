...
def test(model, predictor, g, x, split_edge, evaluator, batch_size):
    ...
    pos_train_pred = get_pred(pos_train_edge)
    pos_valid_pred = get_pred(pos_valid_edge)
    neg_valid_pred = get_pred(neg_valid_edge)
    pos_test_pred = get_pred(pos_test_edge)
    neg_test_pred = get_pred(neg_test_edge)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                results = test(model, predictor, data, data.ndata['feat'], split_edge, evaluator,
                               args.batch_size)
