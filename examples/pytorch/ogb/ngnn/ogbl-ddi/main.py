...
class NGNN_GCNConv(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, output_channels):
        ...
        self.fc_adj2 = Linear(hidden_channels, output_channels)
...
class NGNN_SAGEConv(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, output_channels,
        ...
        self.conv = SAGEConv(input_channels, hidden_channels, reduce)
        self.fc_adj = Linear(hidden_channels, hidden_channels)
        self.fc_adj2 = Linear(hidden_channels, output_channels)
        self.reset_parameters()
...
def main():
    ...
    parser.add_argument('--use_sage', action='store_true', help='If not set, use GCN by default.', default=True)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, data, emb.weight, split_edge,
                         optimizer, args.batch_size)

            if epoch % args.eval_steps == 0:
                results = test(model, predictor, data, emb.weight, split_edge,
