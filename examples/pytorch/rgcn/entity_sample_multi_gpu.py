...
def train(proc_id, device, g, target_idx, labels, num_classes,
          train_idx, inv_target, model):
    ...
    for epoch in range(50):
        ...
        loc_acc_split = evaluate(model, labels, num_classes,
                                 val_loader, inv_target).to(device)
...
def run(proc_id, nprocs, devices, g, data):
    ...
    # training + testing
    train(proc_id, device, g, target_idx, labels, num_classes,
          train_idx, inv_target, model)
    ...
    loc_acc_split = evaluate(model, labels, num_classes,
                             test_loader, inv_target).to(device)
    dist.reduce(loc_acc_split, 0)
    if proc_id == 0:
        acc = loc_acc_split[0] / loc_acc_split[1]
