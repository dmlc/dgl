...
class RGCNEntityDataset(object):
    ...
    def load(self, bfs_level=2, relabel=False):
        ...
        if bfs_level > 0:
            ...
            row, col, edge_type, row_t, col_t = edges.transpose()
            ...
            self.edge_src_t = row_t[eid_to_keep]
            self.edge_dst_t = col_t[eid_to_keep]
        else:
            self.edge_src, self.edge_dst, self.edge_type, self.edge_src_t, self.edge_dst_t = edges.transpose()
        ...
        nodes = np.concatenate([self.edge_src, self.edge_dst])
        nodes_t = np.concatenate([self.edge_src_t, self.edge_dst_t])
        node_ids, unique_indices = np.unique(nodes, return_index=True)
        print(node_ids.shape[0])
        print(unique_indices.shape[0])
        node_tids = nodes_t[unique_indices]
        self.node_tids = np.full((self.num_nodes, ), -1)
        self.node_tids[node_ids] = node_tids
        self.num_of_ntype = np.unique(node_tids).shape[0]
    ...
    if os.path.isfile(edge_file) and os.path.isfile(labels_file) and \
        ...
        with RDFReader(graph_file) as reader:
            ...
            def parse_entity_type(term):
                ...
                if dataset_str == 'am':
                    if isinstance(term, rdf.Literal):
                        ...
                        return '_BNode'
                    entstr = str(term)
                    if entstr.startswith(entity_prefix):
                        sp = entstr.split('/')
                    ...
                    if isinstance(term, rdf.Literal):
                        ...
                        return None
                    entstr = str(term)
                    if entstr.startswith(status_prefix):
                        return None
                    if entstr.startswith(entity_prefix):
                        sp = entstr.split('/')
                        if len(sp) != 7:
            ...
            for i, (s, p, o) in enumerate(reader.triples()):
                src = nodes_dict[s]
                src_t = parse_entity_type(s)
                if src_t is None:
                    continue
                src_tid = _get_id(nt_dict, src_t)
                dst = nodes_dict[o]
                dst_t = parse_entity_type(o)
                if dst_t is None:
                    continue
                dst_tid = _get_id(nt_dict, dst_t)
                assert src < num_node and dst < num_node
                rel = relations_dict[p]
                # relation id 0 is self-relation, so others should start with 1
                edge_list.append((src, dst, 2 * rel + 1, src_tid, dst_tid))
                # reverse relation
                edge_list.append((dst, src, 2 * rel + 2, dst_tid, src_tid))
...
def load_entity(dataset, bfs_level=3, relabel=False):
