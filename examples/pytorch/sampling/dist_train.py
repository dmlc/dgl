...
class DistGraphStoreServer(KVServer):
    def __init__(self, ip_config, server_id, server_data, num_client):
        ...
        num_nodes = np.max(self.part_g.ndata[dgl.NID].numpy()) + 1
        self.g2l = th.zeros((num_nodes,), dtype=th.int64)
        self.g2l[:] = -1
        self.g2l[self.part_g.ndata[dgl.NID]] = th.arange(self.part_g.number_of_nodes())
        if self.get_id() % self.get_group_count() == 0: # master server
            for ndata_name in self.part_g.ndata.keys():
                print('init', ndata_name)
    ...
    def _pull_handler(self, name, lID, target):
        #lID = self.g2l[gID].numpy()
        #gID = gID.numpy()
        #print(gID[lID == -1])
        assert th.sum(lID == -1) == 0
...
class DistGraphStore:
    def __init__(self, ip_config, graph_path):
        ...
        num_nodes = np.max(self.g.ndata[dgl.NID].numpy()) + 1
        partition = th.zeros((num_nodes,), dtype=th.int64)
        ...
        local_nids = np.nonzero((self.g.ndata['part_id'] == self.part_id).numpy())[0]
...
def main(args):
    ...
    train_mask = g.get_ndata('train_mask')
    val_mask = g.get_ndata('val_mask')
    test_mask = g.get_ndata('test_mask')
    print('part {}, train: {}, val: {}, test: {}'.format(g.get_id(),
        th.sum(train_mask), th.sum(val_mask), th.sum(test_mask)), flush=True)

    train_nid = th.LongTensor(np.nonzero(train_mask.numpy())[0])
    test_nid = th.LongTensor(np.nonzero(test_mask.numpy())[0])

    if args.model == "gcn_ns":
        gcn_ns_train(g, args, False, args.n_classes, train_nid, test_nid)
    else:
