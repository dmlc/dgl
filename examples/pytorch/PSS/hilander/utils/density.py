...
__all__ = ['density_estimation', 'density_estimation_soft', 'density_to_peaks', 'density_to_peaks_vectorize']
...
def density_estimation_soft(dists, nbrs, softlabels, **kwargs):
    ''' use supervised density defined on neigborhood
    '''
    num, k_knn = dists.shape
    conf = np.ones((num, ), dtype=np.float32)
    expand_softlabels = np.expand_dims(softlabels, 1).repeat(k_knn, 1)
    ind_array = [softlabels[nbrs][i].dot(expand_softlabels[i]) for i in range(len(expand_softlabels))]
    pos = ((1-dists[:,1:]) * ind_array[:,1:]).sum(1)
    neg = ((1-dists[:,1:]) * (1-ind_array[:,1:])).sum(1)
    conf = (pos - neg) * conf
    conf /= (k_knn - 1)
