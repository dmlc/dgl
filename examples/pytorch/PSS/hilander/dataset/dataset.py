...
from utils import (build_knns, fast_knns2spmat, row_normalize, knns2ordered_nbrs,
                   ...
                   decode, build_next_level, density_estimation_soft)
...
class LanderDatasetSoft(object):
    def __init__(self, features, softlabels, cluster_features=None, k=10, levels=1, faiss_gpu=False):
        self.k = k
        self.gs = []
        self.nbrs = []
        self.dists = []
        ...
        for lvl in range(self.levels):
            ...
            self.nbrs.append(nbrs)
            self.dists.append(dists)
            density = density_estimation_soft(dists, nbrs, softlabels)

            g = self._build_graph(features, cluster_features, softlabels, density, knns)
            self.gs.append(g)
            ...
            new_pred_labels, peaks,\
                global_edges, global_pred_labels, global_peaks = decode(g, 0, 'sim', True,
                                                                        ...
                                                                        global_peaks)
            ids = ids[peaks]
            features, labels, cluster_features = build_next_level(features, softlabels, peaks,
                                                                  global_features, global_pred_labels, global_peaks)
    ...
    def _build_graph(self, features, cluster_features, labels, density, knns):
        ...
        g.ndata['norm'] = torch.FloatTensor(adj_row_sum)
        g.apply_edges(lambda edges: {'raw_affine': edges.data['affine'] / edges.dst['norm']})
        g.apply_edges(lambda edges: {'labels_conn': (edges.src['labels'].dot(edges.dst['labels'])).long()})
        g.apply_edges(lambda edges: {'mask_conn': (edges.src['density'] > edges.dst['density']).bool()})
        return g
