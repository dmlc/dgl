...
def eval_metrics_one_dataset(model, test_dataloader, device, k_vals, opt):
    ...
    with torch.no_grad():
        ...
        # image_paths = [x for x in test_dataloader.dataset.imgs]
        for idx, (input_img, target) in enumerate(final_iter):
        ...
        feature_coll = np.array(nn.functional.normalize(torch.tensor(feature_coll), dim=1, p=2))
...
def get_features(model, test_dataloader, device, split, opt):
    torch.cuda.empty_cache()
    linsize = 29011
    uinsize = 18403
    if opt.iter > 0:
        ...
        with open(os.path.join(opt.data_path, opt.train_ann)) as f:
    ...
    if "all_train" in split:
        if opt.iter > 0:
            ...
            for i in range(len(paths)):
                path = paths[i]
                idxx = path2ids["/home/ubuntu/code/dino/data/inaturalist/"+path]
                path2ids_new["/home/ubuntu/code/dino/data/inaturalist/"+path] = i
                predicted_features[i] = feature_coll[idxx]
                target_labels_new[i] = target_labels[idxx]
    ...
    output_feature_path = os.path.join("/home/ubuntu/code/Smooth_AP/data/" + split + "_inat_features.pkl")
