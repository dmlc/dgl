...
class SoftSmoothAP(torch.nn.Module):
    ...
    def forward(self, preds, softlabels):
        ...
        mask = 1.0 - torch.eye(self.batch_size)
        mask = mask.unsqueeze(dim=0).unsqueeze(dim=0).repeat(self.batch_size, self.class_num, 1, 1)
        print("mask", mask.shape)
        mask = mask * softlabels[:, :, None, None]
        print("mask", mask.shape)
        # compute the relevance scores via cosine similarity of the CNN-produced embedding vectors
        sim_all = compute_aff(preds)
        sim_all_repeat = sim_all.unsqueeze(dim=1).repeat(1, self.batch_size, 1)
        ...
        sim_diff = sim_all_repeat - sim_all_repeat.permute(0, 2, 1)
        # pass through the sigmoid
        sim_sg = mask.cuda() * sigmoid(sim_diff, temp=self.anneal)[:, None, :, :]
        sim_sg = torch.sum(sim_sg, dim=1)
        # compute the rankings
        sim_all_rk = torch.sum(sim_sg, dim=-1) + 1
        print("sim_all_rk", sim_all_rk)
        ...
        softlabel_mask = softlabels_t * softlabels_t_
        softlabel_mask = torch.where(softlabel_mask > 0, 1, 0)
        pos_mask = 1.0 - torch.eye(self.batch_size)
        pos_mask = pos_mask.unsqueeze(dim=0).unsqueeze(dim=0).repeat(self.batch_size, self.class_num, 1, 1)
        pos_mask = pos_mask * softlabel_mask[None, :, :, :] * softlabels[:, :, None, None]
        # compute the relevance scores
        ...
        # pass through the sigmoid
        sim_pos_sg = pos_mask.cuda() * sigmoid(sim_pos_diff, temp=self.anneal)[:, None, :, :]
