...
def give_dataloaders(dataset, trainset, testset, opt, cluster_path=""):
    ...
    if opt.dataset=='Inaturalist':
        if opt.finetune:
            if not opt.onehead:
                datasets = give_inat_datasets_finetune_2head(trainset, testset, cluster_path, opt)
            else:
        ...
        if opt.finetune:
            if not opt.onehead:
                datasets = give_semifungi_datasets_finetune(trainset, testset, cluster_path, opt)
            else:
                datasets = give_semifungi_datasets_finetune_1head(testset, cluster_path, opt)
        else:
            datasets = give_semifungi_datasets(trainset, testset, opt)
    else:
    ...
    for key, dataset in datasets.items():
        if (isinstance(dataset, TrainDatasetsmoothap) or isinstance(dataset, TrainDatasetsmoothapSoft) or
            isinstance(dataset, TrainDatasetsmoothap1Head) or isinstance(dataset, TrainDatasetsmoothap1HeadSoft))\
...
def give_inat_datasets_finetune_2head(cluster_label_path, opt):
    """
    This function generates a training, testing and evaluation dataloader for Metric Learning on the Inaturalist 2018 dataset.
    For Metric Learning, training and test sets are provided by given json files. Will define a train and test split
    So no random shuffling of classes.
    ...
    with open(cluster_label_path, 'rb') as clusterf:
        ...
        for path, idx in path2idx.items():
            if idx < opt.linsize or global_pred_labels[idx] == -1:
                continue
            else:
                if "te/%d" % global_pred_labels[idx] not in cluster_image_dict:
                    cluster_image_dict["te/%d" % global_pred_labels[idx]] = []
                cluster_image_dict["te/%d" % global_pred_labels[idx]].append(path)
    ...
    new_cluster_image_dict = {}
    cluster_ind_ind = 0
    for cate in cluster_image_dict:
        if len(cluster_image_dict[cate]) > 1:
            new_cluster_image_dict["te/%d" % cluster_ind_ind] = cluster_image_dict[cate]
            cluster_ind_ind += 1
    cluster_image_dict = new_cluster_image_dict

    train_dataset = TrainDatasetsmoothap(train_image_dict, opt)
    cluster_dataset = TrainDatasetsmoothap(cluster_image_dict, opt)
    ...
    # val_dataset.conversion         = conversion
    # eval_dataset.conversion        = conversion
    return {'training': train_dataset, 'clustering': cluster_dataset, 'testing': val_dataset, 'evaluation': val_dataset}
...
def give_semifungi_datasets(trainset, testset, opt):
    """
    This function generates a training, testing and evaluation dataloader for Metric Learning on the Inaturalist 2018 dataset.
    For Metric Learning, training and test sets are provided by given json files. Will define a train and test split
    So no random shuffling of classes.
    ...
    with open(os.path.join(opt.source_path, trainset)) as f:
        FileLines = f.readlines()
        FileLines = [x.strip() for x in FileLines]

        for entry in FileLines:
            info = entry.split('/')
            if info[-2] not in train_image_dict:
                train_image_dict[info[-2]] = []
            file_name = ' '.join(entry.split(' ')[:-1])
            train_image_dict[info[-2]].append(os.path.join(opt.source_path,file_name))
    ...
    with open(os.path.join(opt.source_path, testset)) as f:
        ...
        for entry in FileLines:
            info = entry.split('/')
            if info[-2] not in val_image_dict:
                val_image_dict[info[-2]] = []
            file_name = ' '.join(entry.split(' ')[:-1])
            val_image_dict[info[-2]].append(os.path.join(opt.source_path,file_name))
...
def give_semifungi_datasets_finetune(trainset, testset, cluster_label_path, opt):
    """
    This function generates a training, testing and evaluation dataloader for Metric Learning on the Inaturalist 2018 dataset.
    For Metric Learning, training and test sets are provided by given json files. Will define a train and test split
    So no random shuffling of classes.
    ...
    train_image_dict, val_image_dict, cluster_image_dict  = {},{},{}
    with open(cluster_label_path, 'rb') as clusterf:
        path2idx, global_features, global_pred_labels, masks = pickle.load(clusterf)
    ...
    # print("train_image_dict", train_image_dict.keys())
    # print("cluster_image_dict", cluster_image_dict.keys())
...
def give_semifungi_datasets_finetune_1head(testset, cluster_label_path, opt):
    """
    This function generates a training, testing and evaluation dataloader for Metric Learning on the Inaturalist 2018 dataset.
    For Metric Learning, training and test sets are provided by given json files. Will define a train and test split
    So no random shuffling of classes.
    ...
    train_dataset = TrainDatasetsmoothap1Head(train_image_dict, cluster_image_dict, opt)
...
class BaseTripletDatasetSoft(Dataset):
    """
    Dataset class to provide (augmented) correctly prepared training samples corresponding to standard DML literature.
    This includes normalizing to ImageNet-standards, and Random & Resized cropping of shapes 224 for ResNet50 and 227 for
    GoogLeNet during Training. During validation, only resizing to 256 or center cropping to 224/227 is performed.
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            tuple of form (sample_class, torch.Tensor() of input image)
        """
        if self.pars.loss == 'softsmoothap' or self.pars.loss == 'softams':
            if self.is_init:
                # self.current_class = self.avail_classes[idx%len(self.avail_classes)]
                self.is_init = False
            ...
            if not self.is_validation:
                ...
                return self.image_list[idx][1], self.transform(self.ensure_3dim(Image.open(self.image_list[idx][0][0])))
        else:
            if self.is_init:
                self.current_class = self.avail_classes[idx % len(self.avail_classes)]
                self.is_init = False
            if not self.is_validation:
                ...
                return self.image_list[idx][-1][1], self.transform(self.ensure_3dim(Image.open(self.image_list[idx][0])))
...
class TrainDatasetsmoothapSoft(Dataset):
    """
    This dataset class allows mini-batch formation pre-epoch, for greater speed
    ...
    def __init__(self, image_dict, opt):
        ...
        transf_list.extend([
            transforms.RandomResizedCrop(size=224) if opt.arch in ['resnet50',
                                                                   'resnet50_mcn'] else transforms.RandomResizedCrop(
                size=227),
            transforms.RandomHorizontalFlip(0.5)])
        transf_list.extend([transforms.ToTensor(), normalize])
        self.transform = transforms.Compose(transf_list)
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name == 'Inaturalist':
            cls = int(batch_item[0].split('/')[1])
        elif self.dataset_name == 'SoftInaturalist':
            cls = batch_item[1][1]
        else:
            cls = batch_item[0]
        img = Image.open(batch_item[1][0])
        return cls, self.transform(self.ensure_3dim(img))
...
class TrainDatasetsmoothap1HeadSoft(Dataset):
    """
    This dataset class allows mini-batch formation pre-epoch, for greater speed
