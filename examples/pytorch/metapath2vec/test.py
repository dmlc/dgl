...
def trivial_test(test_path, output_file):
    ...
    experiment_times = 1
    percent = 0.05
    url_1 = "https://s3.us-east-2.amazonaws.com/dgl.ai/dataset/googlescholar.8area.venue.label.txt"
    url_2 = "https://s3.us-east-2.amazonaws.com/dgl.ai/dataset/googlescholar.8area.author.label.txt"
    file_name_1 = os.path.join(test_path, "venue_label.txt")
    file_name_2 = os.path.join(test_path, "author_label.txt")
    urllib.request.urlretrieve(url_1, file_name_1)
    ...
    for time in range(experiment_times):
        ...
        clf_venue = LogisticRegression(random_state=0, solver="lbfgs", multi_class="multinomial").fit(venue_training,
                                                                                                      venue_label)
        y_pred_venue = clf_venue.predict(venue_testing)
        clf_author = LogisticRegression(random_state=0, solver="lbfgs", multi_class="multinomial").fit(author_training,
                                                                                                       author_label)
