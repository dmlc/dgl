...
torch::Tensor SpMMSumCsrNaive(torch::Tensor rowptr,
				torch::Tensor cols, torch::Tensor val,
				torch::Tensor matB, torch::Tensor tmp) {
  ...
  //           out_data[row * K + k] += val_data[idx] * B_data[col_data[idx] * K + k];
  //       }
  //     }
  //   }
  // });
