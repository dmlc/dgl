...
/*! \brief Libra - vertexcut based graph partitioning.
  ...
  based on the end vertices, in doing so, it tries to minimized the splitting of
  the graph vertices. In case of conflict Libra assigns an edge to the least
  loaded partition/community. \param[in] nc Number of partitions/communities
...
/*! \brief
  ...
  partitions
   ...
   These dictionaries will be used in the subsequesnt stages to setup tracking
  of split nodes copies across the partition, setting up partition `ndata`
  dictionaries. \param[out] a local src node ID of an edge in a partition
  ...
  node ID in a partition \param[out] ldt_key per partition dict for storing
  global and local node IDs (consecutive) \param[out] gdt_key global dict for
  storing number of local nodes (or split nodes) for a given global node ID
  \param[out] gdt_value global dict, stores local node IDs (due to split) across
  partitions for a given global node ID \param[out] node_map keeps track of
  range of local node IDs (consecutive) given to the nodes in the partitions
...
/*! \brief sets up the 1-level tree among the clones of the split-nodes.
  ...
  across all the partitions \param[in] gdt_value global dict for assigning
  consecutive node IDs to nodes across all the partition \param[out] lrtensor
  keeps the root node ID of 1-level tree \param[in] nc number of
  partitions/communities \param[in] Nn number of nodes in the input graph
 */
void Libra2dglSetLR(
...
/*!
  ...
  also, for each node in a partition, it gathers the data (feats, label, trian,
  test) from input graph. \param[out] feat node features in current partition c
  ...
  nodes) for a given global node ID \param[out] gdt_value global dict, stores
  local node IDs (due to split) across partitions for a given global node ID
  \param[in] node_map keeps track of range of local node IDs (consecutive) given
  to the nodes in the partitions \param[out] lr 1-level tree marking for local
  split nodes \param[in] lrtensor global (all the partitions) 1-level tree
