 ...
 */
 ...
 public:
     ...
     workspace_memory_alloc(ctx_t ctx) : ctx(ctx) {}
     ...
     std::unique_ptr<T, workspace_memory_alloc> alloc_unique(std::size_t size) {
         return std::unique_ptr<T, workspace_memory_alloc>((T *)runtime::DeviceAPI::Get(*ctx)->AllocWorkspace(*ctx, sizeof(T) * size), *this);
     }
 
     char *allocate(std::ptrdiff_t size) {
         return (char *)runtime::DeviceAPI::Get(*ctx)->AllocWorkspace(*ctx, size);
     }
 ...
 struct TransformOpImp {
     ...
     auto operator() (IdType idx) {
         ...
         const auto in_idx = indptr[row] + idx - subindptr[in_row];
         const auto data = data_arr ? data_arr[in_idx] : in_idx;
         return thrust::make_tuple(in_row, row, u, data, 1 / min((FloatType)1, c * ps));
     }
 };
 ...
 template <typename IdType, typename FloatType, typename ps_t>
 ...
 /**
 * @brief Compute the indegrees of each row from which sampling will be performed.
 *
 * @tparam IdType The type of node and edge indexes.
 * @param num_rows The number of rows to pick.
 * @param rows The set of rows to pick.
 * @param indptr The index where each row's edges start.
 * @param in_deg The size of each row in the sampled matrix, as indexed by
 * `rows` (output).
 */
 template <typename IdType, typename FloatType>
 ...
 __global__ void _CSRRowWiseLayerSampleDegreeKernel(
     ...
     while (out_row < last_row) {
         ...
         const auto inv_d = ONE / d;
 
         // slightly better than NS
         const FloatType var_target = num_picks < d ? d * (FloatType)1 * d / num_picks : d;
 ...
 std::pair<COOMatrix, FloatArray> CSRLaborSampling(CSRMatrix mat,
     ...
     const IdType num_rows = rows_1->shape[0];
     IdType * rows = static_cast<IdType*>(rows_1->data);
 ...
 
     {
         ...
         const dim3 grid((num_rows + BLOCK_SIZE - 1) / BLOCK_SIZE);
         CUDA_KERNEL_CALL((_CSRRowWiseDegreeKernel<IdType, FloatType>),
             grid, block, 0, stream,
             num_rows, num_picks, rows, indptr, in_deg.get(), cs);
     }
     ...
     if (importance_sampling) { // extracts the onehop neighborhood cols to a contigous range into hop_1
         const dim3 block(BLOCK_SIZE);
     ...
     if (importance_sampling) {
         ...
         if (use_hashtable) {
 ...
 
             {
                 ...
                 CUDA_CALL(cub::DeviceRadixSort::SortKeys(nullptr, temp_storage_bytes, hop_b, hop_size, 0, max_log_num_vertices, stream));
 
                 auto temp = allocator.alloc_unique<char>(temp_storage_bytes);
 
                 CUDA_CALL(cub::DeviceRadixSort::SortKeys(temp.get(), temp_storage_bytes, hop_b, hop_size, 0, max_log_num_vertices, stream));
             }
             ...
             {
                 std::size_t temp_storage_bytes = 0;
                 CUDA_CALL(cub::DeviceRunLengthEncode::Encode(nullptr, temp_storage_bytes, hop_b.Current(), hop_unique.get(), hop_counts.get(), hop_unique_size.get(), hop_size, stream));
 
                 auto temp = allocator.alloc_unique<char>(temp_storage_bytes);
 
                 CUDA_CALL(cub::DeviceRunLengthEncode::Encode(temp.get(), temp_storage_bytes, hop_b.Current(), hop_unique.get(), hop_counts.get(), hop_unique_size.get(), hop_size, stream));
         ...
         for (int iters = 0; iters < importance_sampling || importance_sampling < 0; iters++) {
             ...
             
             {
                 ...
                 CUDA_KERNEL_CALL((_CSRRowWiseLayerSampleDegreeKernel<IdType, FloatType, BLOCK_CTAS, TILE_SIZE>),
                     grid, block, 0, stream,
                     num_picks,
                     num_rows,
                     rows,
     ...
     FloatArray picked_imp = NewFloatArray(importance_sampling ? hop_size : 1, ctx, sizeof(FloatType) * 8);
     ...
     IdType num_edges;
     {
         ...
         if (importance_sampling) {
             ...
             auto transformed_output = thrust::make_transform_output_iterator(output, TransformOpImp<IdType, FloatType>{hop_1, probs_found.get(), idx_coo, rows, cs, indptr, subindptr, data});
             ...
             auto transformed_output = thrust::make_transform_output_iterator(output, TransformOp<IdType>{idx_coo, rows, indptr, subindptr, indices, data});
             ...
             const FloatType a = std::cos(pi * l / 2);
             const FloatType b = std::sin(pi * l / 2);
             const auto pred = StencilOpFused<IdType, FloatType, decltype(one)>{seeds[0], seeds[num_seeds - 1], a, b, idx_coo, cs, one, subindptr, rows, indptr, indices, nids};
     ...
     if (importance_sampling) {
         ...
         auto new_end = thrust::reduce_by_key(exec_policy, picked_inrow.get(), picked_inrow.get() + num_edges, one, keys.get(), ds.get());
         thrust::reduce_by_key(exec_policy, picked_inrow.get(), picked_inrow.get() + num_edges, picked_imp_data, keys.get(), ws.get());
         {
         ...
         }
         {
             ...
             auto transformed_input = thrust::make_transform_iterator(input, thrust::make_zip_function(TransformOpMean<IdType, FloatType>{ds_2.get(), ws_2.get()}));
