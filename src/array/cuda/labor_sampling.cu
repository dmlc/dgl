...
__global__ void _CSRRowWiseLayerSampleDegreeKernel(
    ...
    while (out_row < last_row) {
        ...
        const FloatType var_target = num_picks < d ? (ds ? ds[row] * ds[row] * (ONE / num_picks - ONE / d) + d2s[row] : d * (FloatType)1 * d / num_picks) : (ds ? d2s[row] : d);
...
std::pair<COOMatrix, FloatArray> CSRLaborSampling(CSRMatrix mat,
    ...
    
    {
        ...
        thrust::for_each(exec_policy, iota, iota + num_rows, DegreeFunc<IdType, FloatType>{num_picks, rows, indptr, weights ? ds : nullptr, in_deg.get(), cs});
    }
    ...
    FloatArray picked_imp = importance_sampling || weights ? NewFloatArray(hop_size, ctx, sizeof(FloatType) * 8) : NullArray();
    ...
    IdType num_edges;
    {
        ...
        if (importance_sampling) {
            ...
            if (weights) {
                ...
                const auto pred = StencilOpFused<IdType, FloatType, decltype(one), decltype(one)>{seeds[0], seeds[num_seeds - 1], a, b, idx_coo, cs, one, one, subindptr, rows, indptr, indices, nids};
                num_edges = thrust::copy_if(exec_policy, iota, iota + hop_size, iota, transformed_output, pred) - transformed_output;
            }
        }
    }
