...
NDArray IndexSelectCPUFromGPU(NDArray array, IdArray index) {
  ...
  if (num_feat == 1) {
      ...
      CUDA_KERNEL_CALL(IndexSelectSingleKernelUVM, nb, nt, 0,
          thr_entry->stream, array_data, idx_data, arr_len, len, ret_data);
  } else {
      dim3 block(256, 1);
      ...
      if (num_feat * sizeof(DType) < 2 * CACHE_LINE_SIZE) {
        CUDA_KERNEL_CALL(IndexSelectMultiKernelUVM, grid, block, 0,
            thr_entry->stream, array_data, num_feat, idx_data,
            arr_len, len, ret_data);
      } else {
        CUDA_KERNEL_CALL(IndexSelectMultiKernelUVMAligned, grid, block, 0,
            thr_entry->stream, array_data, num_feat, idx_data,
            arr_len, len, ret_data);
      }
  }
