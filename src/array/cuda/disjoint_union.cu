...
template COOMatrix DisjointUnionCoo<kDLCUDA, int32_t>(const std::vector<COOMatrix>& coos);
template COOMatrix DisjointUnionCoo<kDLCUDA, int64_t>(const std::vector<COOMatrix>& coos);
