...
template <typename IdType, typename DType, typename Op>
void Edge_softmax_csr_test(const BcastOff& bcast, const CSRMatrix& csr, NDArray ufeat,
                NDArray efeat, NDArray out) {
  const bool has_idx = !IsNullArray(csr.data);
  const IdType* indptr = static_cast<IdType*>(csr.indptr->data);
  const IdType* edges = 
...
template <typename IdType, typename DType, typename Op>
void Edge_softmax_csr_test_back(const BcastOff& bcast, const CSRMatrix& csr, NDArray out,
