...
class UnitGraph::COO : public BaseHeteroGraph {
 public:
  ...
  COO(GraphPtr metagraph, const aten::COOMatrix& coo)
    ...
    if (!COOHasData(coo))
      // TODO(zihao): verify the following statement.
...
HeteroGraphPtr UnitGraph::AsNumBits(HeteroGraphPtr g, uint8_t bits) {
  if (g->NumBits() == bits) {
    ...
    if (bg->coo_)
      new_coo = COOPtr(new COO(bg->GetCOO()->AsNumBits(bits)));
    CSRPtr new_incsr = nullptr;
    if (bg->in_csr_)
      new_incsr = CSRPtr(new CSR(bg->GetInCSR()->AsNumBits(bits)));
    CSRPtr new_outcsr = nullptr;
    if (bg->out_csr_)
      new_outcsr = CSRPtr(new CSR(bg->GetOutCSR()->AsNumBits(bits)));
...
HeteroGraphPtr UnitGraph::CopyTo(HeteroGraphPtr g, const DLContext& ctx) {
  ...
  COOPtr new_coo = nullptr;
  if (bg->coo_)
    new_coo = COOPtr(new COO(bg->GetCOO()->CopyTo(ctx)));
  CSRPtr new_incsr = nullptr;
  if (bg->in_csr_)
