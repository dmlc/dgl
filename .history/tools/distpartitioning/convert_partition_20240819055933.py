...
def create_dgl_object(
    ...
    if use_graphbolt:
        ...
        shape=int(max(max(part_local_dst_id)+1,max(part_local_src_id)+1))
        adj_matrix = dglsp.from_coo(th.tensor(part_local_src_id), th.tensor(part_local_dst_id),shape=(shape,shape))
