...
DGL NN模块是用户搭建GNN模型的基本模块。根据后端使用的深度神经网络框架，
...
在DGL NN模块中，构造函数中的参数注册和前向函数中的张量操作与后端框架一样。用这种方式，DGL的代码可以无缝嵌入到后端框架的代码中。
...
DGL已经整合了很多常用的Conv层, Dense Conv层, Global Pooling层和Utility模块。欢迎给DGL贡献更多模块！
...
这个部分将使用PyTorch作为后端，SAGEConv作为例子来介绍如何搭建用户自己的DGL NN模块。
