...
class NodeIdxCollator(NodeCollator):
    ...
    indices : Tensor or dict[ntype, Tensor]
        The target indices to comput outputs.
    block_sampler : dgl.dataloading.BlockSampler
        The neighborhood sampler.
    ...
    >>> sampler = dgl.dataloading.MultiLayerNeighborSampler([15, 10, 5])
    >>> collator = dgl.dataloading.NodeIdxCollator(g, train_nid, sampler)
    >>> dataloader = torch.utils.data.DataLoader(
    ...     collator.dataset, collate_fn=collator.collate,
    ...     batch_size=1024, shuffle=True, drop_last=False, num_workers=4)
    >>> for input_nodes, output_nodes, seed_idx, blocks in dataloader:
    ...     train_on(input_nodes, output_nodes, seed_idx, blocks)
    """
    def __init__(self, g, nids, indices, block_sampler):
        self.g = g
        self._is_distributed = isinstance(g, DistGraph)
        if not isinstance(nids, Mapping):
            assert len(g.ntypes) == 1, \
        ...
        if isinstance(indices, Mapping):
            assert isinstance(nids, Mapping), "indices is a dict, nids should also be a dict"
            for key in indices.keys():
                assert key in nids.keys(), \
                    "indices are provided for {}, nids should also be provided".format(key)
    ...
    def collate(self, items):
        ...
        if not self._is_distributed:
            if isinstance(items, dict):
                ...
                items = utils.prepare_tensor(self.g, items, 'items')
                sample_items = self.nids[items]
        if isinstance(items, dict):
            sample_items = {key: self.nids[val] for key, val in items.items()}
        else:
            sample_items = self.nids[items]
