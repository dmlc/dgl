...
def node_subgraph(graph, nodes, store_raw_ids=True):
    ...
    * Copy the features of the extracted nodes and edges to the resulting graph.
    ...
    return _create_hetero_subgraph(graph, sgi, induced_nodes, induced_edges, store_raw_ids)

DGLHeteroGraph.subgraph = utils.alias_func(node_subgraph)

def edge_subgraph(graph, edges, preserve_nodes=False, store_raw_ids=True):
...
def _create_hetero_subgraph(parent, sgi, induced_nodes, induced_edges, store_raw_ids=True):
    ...
    DGLGraph
        Graph
    """
    node_frames = utils.extract_node_subframes(parent, induced_nodes, store_raw_ids)
    edge_frames = utils.extract_edge_subframes(parent, induced_edges, store_raw_ids)
