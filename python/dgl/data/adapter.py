...
class AsNodePredDataset(DGLDataset):
    ...
    def process(self):
        has_mask = all([mask_name in self.g.ndata for mask_name in (
            "train_mask", "val_mask", "test_mask")])
        if has_mask:
            if self.verbose:
                print("Dataset already have train/val/test masks.")
        else:
            if self.verbose:
...
class AsEdgePredDataset(DGLDataset):
    ...
    def __init__(self,
        ...
        super().__init__(dataset.name + '-as-edgepred', **kwargs)
    ...
    def load(self):
        gs, tensor_dict = utils.load_graphs(
            os.path.join(self.save_path, 'graph.bin'))
        self.g = gs[0]
        self.train_graph = self.g
    ...
    def save(self):
        ...
        utils.save_graphs(os.path.join(self.save_path, 'graph.bin'), [
