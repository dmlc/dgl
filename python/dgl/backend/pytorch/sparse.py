import torch as th
from ...sparse import _gspmm as _gspmm_no_grad
from ...sparse import _gsddmm as _gsddmm_no_grad
...
class GSpMM(th.autograd.Function):
    ...
    def forward(ctx, gidx, op, reduce_op, X, Y):
        out, (argX, argY) = _gspmm_no_grad(gidx, op, reduce_op, X, Y)
    ...
    def backward(ctx, dZ):
        ...
        if op != 'copy_rhs' and ctx.needs_input_grad[3]:
            ...
            if reduce_op == 'sum':
                if op in ['mul', 'div']:
                    dX = _gspmm(g_rev, 'mul', 'sum', dZ, _muldiv(op, Y))
                elif op in ['add', 'sub']:
                    dX = _gspmm(g_rev, 'copy_lhs', 'sum', dZ, Y)
                elif op == 'copy_lhs':
                    dX = _gspmm(g_rev, 'copy_lhs', 'sum', dZ, None)
            else:
...
class GSDDMM(th.autograd.Function):
    ...
    def forward(ctx, gidx, op, X, Y, lhs_target, rhs_target):
        out = _gsddmm_no_grad(gidx, op, X, Y, lhs_target, rhs_target)
            ...
            if lhs_target in ['u', 'v']:
                ...
                if op in ['add', 'sub', 'copy_lhs']:
                    ...
                    if rhs_target == lhs_target:
                        dX = _gspmm(_gidx, 'copy_rhs', 'sum', None, dZ) * _muldiv(op, Y)
                    elif rhs_target == 'e':
                        dX = _gspmm(_gidx, 'copy_rhs', 'sum', None, dZ * _muldiv(op, Y))
                    else:  # rhs_target = !lhs_target
                        dX = _gspmm(_gidx, 'mul', 'sum', _muldiv(op, Y), dZ)
            else:  # lhs_target == 'e'
                if op in ['add', 'sub', 'copy_lhs']:
                    dX = dZ
                else:  # mul, div, dot
        ...
        if op != 'copy_lhs' and ctx.needs_input_grad[3]:
            if rhs_target in ['u', 'v']:
                ...
                if op in ['add', 'sub', 'copy_rhs']:
                    dY = _gspmm(_gidx, 'copy_rhs', 'sum', None, _addsub(op, dZ))
                else:  # mul, div, dot
                    if lhs_target == rhs_target:
                        dY = _gspmm(_gidx, 'copy_rhs', 'sum', None, dZ) * X
                    elif lhs_target == 'e':
                        dY = _gspmm(_gidx, 'copy_rhs', 'sum', None, dZ * X)
                    else:  # rhs_target = !lhs_target
                        dY = _gspmm(_gidx, 'mul', 'sum', X, dZ)
                    if op == 'div': dY = -dY / (Y ** 2)
            else:
                if op in ['add', 'sub', 'copy_rhs']:
                    dY = _addsub(op, dZ)
...
def _gspmm(gidx, op, reduce_op, lhs_data, rhs_data):
    return GSpMM.apply(gidx, op, reduce_op, lhs_data, rhs_data)

def _gsddmm(gidx, op, lhs_data, rhs_data, lhs_target, rhs_target):
    return GSDDMM.apply(gidx, op, lhs_data, rhs_data, lhs_target, rhs_target)

def gspmm(g, op, reduce_op, lhs_data, rhs_data):
    return _gspmm(g._graph, op, reduce_op, lhs_data, rhs_data)

def gsddmm(g, op, lhs_data, rhs_data, lhs_target='u', rhs_target='v'):
    return _gsddmm(g._graph, op, lhs_data, rhs_data, lhs_target, rhs_target)
