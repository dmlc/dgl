import torch as th
from ...sparse import _gspmm as gspmm_no_grad
from ...sparse import _gsddmm as gsddmm_no_grad
...
class GSpMM(th.autograd.Function):
    ...
    def forward(ctx, gidx, op, reduce_op, X, Y):
        out, (argX, argY) = gspmm_no_grad(gidx, op, reduce_op, X, Y)
        ...
        if op != 'copy_lhs' and ctx.needs_input_grad[4]:
            if reduce_op == 'sum':
                if op == 'mul' and _need_reduce_last_dim(X, Y):
                    dY = gsddmm(gidx, 'dot', X, dZ, 'u', 'v')
                elif op in ['mul', 'div']:
                    dY = gsddmm(gidx, 'mul', X, dZ, 'u', 'v')
                    if op == 'div': dY = -dY / (Y ** 2)
                elif op in ['add', 'sub', 'copy_rhs']:
                    dY = gsddmm(gidx, 'copy_rhs', X, _addsub(op, dZ), 'u', 'v')
            else:
...
class GSDDMM(th.autograd.Function):
    ...
    def forward(ctx, gidx, op, X, Y, lhs_target, rhs_target):
        out = gsddmm_no_grad(gidx, op, X, Y, lhs_target, rhs_target)
