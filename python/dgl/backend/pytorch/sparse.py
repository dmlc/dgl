...
from ...sparse import _csrmm, _csrsum, _csrmask, get_hs_id
...
class GSpMM_hetero(th.autograd.Function):
    ...
    def forward(ctx, g, op, reduce_op, X_len, *feats): # feats = lhs_data + rhs_data
        ...
        # checking the first relation to decide for all the relations
        if not spmm_cache_X(op, reduce_op, req_grad_X[src_id], req_grad_Y[dst_id]):
            X = tuple([None] * X_len) # None
        if not spmm_cache_Y(op, reduce_op, req_grad_X[src_id], req_grad_Y[dst_id]):
            Y = tuple([None] * len(Y))
...
class GSDDMM_hetero(th.autograd.Function):
    ...
    def forward(ctx, g, op, X_len, lhs_target, rhs_target, *feats): # feats = X+Y
        ...
        req_grad_Y = tuple([Y[i].requires_grad if Y[i] is not None else False
            for i in range(len(Y))])
        lhs_id = get_hs_id(g, g.canonical_etypes[0], lhs_target)
        rhs_id = get_hs_id(g, g.canonical_etypes[0], rhs_target)
        if not sddmm_cache_X(op, req_grad_X[lhs_id], req_grad_Y[rhs_id]):
            X = tuple([None] * len(X))
        if not sddmm_cache_Y(op, req_grad_X[lhs_id], req_grad_Y[rhs_id]):
            Y = tuple([None] * len(Y))
