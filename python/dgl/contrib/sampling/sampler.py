...
class RelationTypeCentralEdgeSampler(object):
    '''Edge sampler for link prediction with centralized relation types in 
    each minibatch. 
    ...
    belong to a extremely small subset of relations. This will help reduce the 
    memory footprints inside a minibatch for models with relation specific 
    params (e.g. transR).

    How it works: 
        1. It sort the whole seed edges according to their edge types (passed 
           through relations tensor)
        2. We divide the whole seed edges into chunks, each chunk will contain
           batch_size of data. And as edges are sorted, the num of relation types
           inside each chunk is minimized by default.
        3. Each fetch will randomly select a chunk to return.
    ...
    the tail node of an edge. When we corrupt the head node of an edge, we 
    randomly sample a node from the entire graph as the head node. It's possible 
    the constructed edge exists in the graph. By default, the implementation 
    doesn't check if the sampled negative edge exists in a graph. However, a user 
    can exclude positive edges from negative edges by specifying 'exclude_positive=True'. 
    When negative edges are created, a batch of negative edges are also placed in 
    a subgraph.
    ...
    Examples
    --------
    >>> for pos_g, neg_g in RelationTypeCentralEdgeSampler(g, batch_size=10, relations=g.edata['type']):
...
class RelationPartitionEdgeSampler(object):
    ...
    edge exists in the graph. By default, the implementation 
    doesn't check if the sampled negative edge exists in a graph. However, a user 
    can exclude positive edges from negative edges by specifying 'exclude_positive=True'. 
    When negative edges are created, a batch of negative edges are also placed in 
    a subgraph.
