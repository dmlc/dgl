    ...
    def forward(self, g, x, ntype, etype, *, presorted=False):
        ...
        with g.local_scope():
            if isinstance(ntype, list):
                k = self.linear_k(x[0], ntype[0], presorted).view(-1, self.num_heads, self.head_size)
                q = self.linear_q(x[1], ntype[1], presorted).view(-1, self.num_heads, self.head_size)
                v = self.linear_v(x[0], ntype[0], presorted).view(-1, self.num_heads, self.head_size)
            else:
