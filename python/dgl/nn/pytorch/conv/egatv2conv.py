...
class EGATv2Conv(nn.Module):
    ...
    def __init__(self,
        ...
        self._in_src_node_feats, self._in_dst_node_feats = expand_as_pair(in_node_feats) #R
        self._out_node_feats = out_node_feats
        self._out_edge_feats = out_edge_feats
        
        if isinstance(in_node_feats, tuple): #R
    ...
    def reset_parameters(self):
        ...
        init.xavier_normal_(self.fc_node.weight, gain=gain) #R replace
    ...
    def forward(self, graph, nfeats, efeats, get_attention=False):
        ...
        with graph.local_scope():
            ...
            graph.srcdata['h_out'] = self.fc_node_src(nfeats_src).view(-1, self._num_heads, #R
                                                             self._out_node_feats)
            # calc weighted sum
            graph.update_all(fn.u_mul_e('h_out', 'a', 'm'),
                             fn.sum('m', 'h_out'))

            h_out = graph.dstdata['h_out'].view(-1, self._num_heads, self._out_node_feats) #R
            if get_attention:
