...
class Metapath2vec(nn.Module):
    """
    Metapath2vec, a scalable representation learning model for heterogeneous networks
    from KDD’17, August <https://dl.acm.org/doi/pdf/10.1145/3097983.3098036>
    Parameters
    ----------
    g :DGLHeteroGraph
    ...
    metapath :List[str]
        The metapath is described as a list of strings for relation edges between different
        types of nodes. It is a  sequence of edges which denotes transition  for different
        types of nodes at each step. Note that the node type at the start and end of the
        metapath are commonly the same.
    window_size :int
        The context size which is considered for positive samples.
    min_count :int , optional
        The number of node counts that occur less than  min_count  value in all meta-paths
        will be discarded before random walks. (default: 0)
    ...
    def _extract(self):
        ...
        if self.nid2word is None:
            for nodetype in self.g.ntypes:
                self.type2id[nodetype] = dict()
                for idx in range(self.g.num_nodes(nodetype)):
                    self.type2id[nodetype][idx] = wid
                    wid += 1
        else:
            for nodetype in self.g.ntypes:
                self.type2id[nodetype] = dict()
        ...
        self.word_count = self.g.number_of_nodes()
    ...
    def _initTableNegatives(self):
        # get a table for negative sampling, if word with index 2 appears twice, then 2
    ...
    def _generate_sample(self, batches):
        ...
        for batch in batches:
            if len(batch) < 0:
                continue
            word_ids = [w for w in batch if w in self.word_frequency
            ...
            for i, u in enumerate(word_ids):
                for j, v in enumerate(
                    ...
                    if i == j:
                        continue
                    pair_catch.append((u, v, self._getNegatives(self.negative_samples_size)))
    ...
    def loader(self, **kwargs):
        r"""Returns the data loader that contains center node ，positive context node，
