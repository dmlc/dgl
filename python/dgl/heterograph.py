...
class DGLHeteroGraph(object):
    ...
    * To create a graph from other data sources, use ``dgl.*`` create ops. See
    ...
    def is_bidirected(self, etype=None, eweight=None):
        ...
        ----------
        etype : str or (str, str, str), optional
            The type name of the edges. It can be omitted if the graph has a single edge type.
            The allowed type name formats are:

            * ``(str, str, str)`` for source node type, edge type and destination node type.
            * or one ``str`` edge type name if the name can uniquely identify a
              triplet format in the graph.
        eweight : str, optional
            The feature name of :attr:`edata` or :attr:`edges.data` to retrieve the edge weights.

        Returns
        -------
        bool
            True if the graph is bidirected.
        ...
        Case1: Check a graph without edge weights

        >>> g = dgl.graph(([0, 1, 2], [1, 2, 3]))
        >>> g.is_bidirected()
        False
        >>> g = dgl.graph(([0, 1, 2, 0], [1, 0, 0, 2]))
        True

        Case2: Check a weighted graph

        >>> import torch
        >>> g.edata['w'] = torch.tensor([0.1, 0.1, 0.2, 0.2])
        >>> g.is_bidirected(eweight='w')
        True
        >>> g.edata['w'] = torch.tensor([0.1, 0.3, 0.2, 0.2])
        >>> g.is_bidirected(eweight='w')
        False
        """
        ...
        src, dst = self.edges(etype=c_etype)
        num_nodes = self.num_nodes(utype)
