...
def metis_partition_assignment(g, k, balance_type='edge', n_hidden=None, objtype='cut'):
    ...
    * `balance_edges` balances the number of edges in each partition.
    ...
    if balance_type == 'edge':
        vwgt.append(F.astype(g.in_degrees(), F.int64))
    elif balance_type == 'workload':
        vwgt.append(F.astype(g.in_degrees() + n_hidden * 3, F.int64))
    else:
