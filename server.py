...
def start_server_bench(server_id, ip_config, num_servers, num_clients, server_state,
    ...
    assert net_type in (
        'socket'), 'net_type (%s) can only be \'socket\'' % net_type
    ...
    # main service loop
    while True:
        msg = rpc.recv_rpc_message(0)
        rpc.send_rpc_message(msg, 0)
...
def main():
    ...
    start_server_bench(server_id=0,
                       ip_config=ip_config,
                       num_servers=1,
                       num_clients=1,
