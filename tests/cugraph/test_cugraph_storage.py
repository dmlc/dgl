...
def create_gs_heterogeneous_dgl_eg():
    ...
    for e in df["etype"].unique().values_host:
        subset_df = df[df["etype"] == e][["src", "dst", "edge_feat"]].reset_index(drop=True)
        gs.add_edge_data(
            subset_df,
            ["src", "dst"],
            feat_name="edge_feat",
    ...
    for n in df["ntype"].unique().values_host:
        ...
        gs.add_node_data(
                subset_df, "node_id", feat_name="node_feat", ntype=n
        )
...
def create_dgl_graph():
    ...
    graph_data = {
            ('nt.a', 'connects', 'nt.b'): (th.tensor([0,1,2]), th.tensor([0,1,2])),
            ('nt.a', 'connects', 'nt.c'): (th.tensor([0,1,2]), th.tensor([0,1,2])),
            ('nt.c', 'connects', 'nt.c'): (th.tensor([1,3,4,5]), th.tensor([0,0,0,0]))
            }
...
def test_sampling_len_cugraph():
    ...
    for fanout in [1,2,3,-1]:
        for ntype in ['nt.a','nt.b', 'nt.c']:
            for d in ['in', 'out']:
                assert_same_sampling_len(dgl_g, cugraph_gs, nodes={ntype:[0]}, fanout=fanout, edge_dir=d)
