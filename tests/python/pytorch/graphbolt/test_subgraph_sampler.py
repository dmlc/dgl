...
@pytest.mark.parametrize("labor", [False, True])
def test_test_SubgraphSampler_without_dedpulication_Homo(labor):
    graph = dgl.graph(
    ...
    for data in datapipe:
        for step, sampled_subgraph in enumerate(data.sampled_subgraphs):
            ...
            assert torch.equal(
                sampled_subgraph.node_pairs["indice"], compacted_indice[step]
            )
            assert torch.equal(
                sampled_subgraph.node_pairs["indptr"], indptr[step]
            )
            assert torch.equal(
...
@pytest.mark.parametrize("labor", [False, True])
def test_test_SubgraphSampler_without_dedpulication_Hetero(labor):
    graph = get_hetero_graph()
    itemset = gb.ItemSetDict(
    ...
    node_pairs = [
        {
            "n1:e1:n2": {
                ...
                "indice": torch.tensor([4, 5, 6, 7]),
            },
            "n2:e2:n1": {
                "indptr": torch.tensor([0, 2, 4, 6, 8]),
                "indice": torch.tensor([2, 3, 4, 5, 6, 7, 8, 9]),
            },
        },
        {
            "n1:e1:n2": {
                "indptr": torch.tensor([0, 2, 4]),
                "indice": torch.tensor([0, 1, 2, 3]),
            },
            "n2:e2:n1": {
                "indptr": torch.tensor([0]),
                "indice": torch.tensor([], dtype=torch.int64),
            },
        },
    ]
    ...
    for data in datapipe:
        for step, sampled_subgraph in enumerate(data.sampled_subgraphs):
            ...
            for etype in ["n1:e1:n2", "n2:e2:n1"]:
                assert torch.equal(
                    sampled_subgraph.node_pairs[etype]["indice"],
                    node_pairs[step][etype]["indice"],
                )
                assert torch.equal(
