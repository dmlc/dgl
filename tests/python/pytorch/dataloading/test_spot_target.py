...
def _find_edges_to_exclude(g, pair_eids, degree_threshold):
    ...
    low_degree_pair_eids = torch.cat([low_degree_pair_eids, low_degree_pair_eids + 1000])
    return low_degree_pair_eids


@pytest.mark.parametrize("degree_threshold", [1, 2, 3, 4, 5])
@pytest.mark.parametrize("batch_size", [1, 10, 50])
def test_spot_target_excludes(    
    degree_threshold,
    batch_size
):
    g, reverse_eids, seed_edges = _create_homogeneous()
    sampler = dgl.dataloading.MultiLayerFullNeighborSampler(1)
    low_degree_excluder = dgl.dataloading.SpotTarget(g, degree_threshold=degree_threshold)
    sampler = dgl.dataloading.as_edge_prediction_sampler(sampler, exclude=low_degree_excluder, reverse_eids=reverse_eids,
    ...
    for i, (input_nodes, pair_graph, neg_pair_graph, blocks) in enumerate(dataloader):
...
if __name__ == "__main__":
    test_spot_target_excludes(
        degree_threshold=2, batch_size=10
    )
