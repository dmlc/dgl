...
if [ "$2" == "cpu" ]; then
    ...
    python3 train.py --model DistMult --dataset FB15k --batch_size 128 \
        --neg_sample_size 16 --hidden_dim 100 --gamma 500.0 --lr 0.1 --max_step 100 \
        --batch_size_eval 16 --gpu 0 --valid --test -adv --mix_cpu_gpu --eval_percent 0.01 \
        --async_update --save_emb DistMult_FB15k_emb \
        --data_path /data/kg || fail "run mix CPU/GPU DistMult"
    ...
    if [ "$1" == "pytorch" ]; then
        # multi process training DistMult
        python3 train.py --model TransR --dataset FB15k --batch_size 128 \
        --neg_sample_size 16 --hidden_dim 100 --gamma 500.0 --lr 0.1 --max_step 100 \
        --batch_size_eval 16 --num_proc 2 --gpu 0 --valid --test -adv --eval_interval 30 --eval_percent 0.01 \
        --data_path /data/kg --mix_cpu_gpu --rel_part --async_update \
