...
def start_client(num_clients, server_count):
    ...
    dgl.distributed.connect_to_server(ip_config='kv_ip_config.txt', server_count=server_count)
    # Init kvclient
...
def start_client_mul_role(i, num_clients, server_count):
    # Note: connect to server first !
    dgl.distributed.connect_to_server(ip_config='kv_ip_mul_config.txt', server_count=server_count)
    # Init kvclient
    if i % 2 == 0:
        kvclient = dgl.distributed.KVClient(ip_config='kv_ip_mul_config.txt', server_count=server_count, role='trainer')
    else:
        kvclient = dgl.distributed.KVClient(ip_config='kv_ip_mul_config.txt', server_count=server_count, role='sampler')
...
def test_kv_multi_role():
    ...
    for i in range(num_servers):
        ...
        pserver_list.append(pserver)
    for i in range(num_clients):
        pclient = ctx.Process(target=start_client_mul_role, args=(i, num_clients, num_servers))
        pclient.start()
        pclient_list.append(pclient)
    for i in range(num_clients):
        pclient_list[i].join()
