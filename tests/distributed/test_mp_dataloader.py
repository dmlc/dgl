...
def start_server(rank, ip_config, part_config, disable_shared_mem, num_clients, keep_alive=False):
    print('server: #clients=' + str(num_clients))
    g = DistGraphServer(rank, ip_config, 1, num_clients,
                        part_config, disable_shared_mem=disable_shared_mem,
                        graph_format=['csc', 'coo'], keep_alive=keep_alive)
    g.start()


def start_dist_dataloader(rank, ip_config, part_config, num_server,
        drop_last, orig_nid, orig_eid, group_id=0):
    import dgl
    import torch as th
    ...
    # Create sampler
    sampler = dgl.dataloading.NeighborSampler(dist_graph, [5, 10],
...
def test_standalone():
    ...
    with tempfile.TemporaryDirectory() as test_dir:
        ...
        orig_nid, orig_eid = partition_graph(g, 'test_sampling', num_parts, test_dir,
                                            num_hops=num_hops, part_method='metis', reshuffle=True,
                                            return_mapping=True)
        part_config = os.path.join(test_dir, 'test_sampling.json')
        os.environ['DGL_DIST_MODE'] = 'standalone'
        try:
            start_dist_dataloader(0, ip_config, part_config, 1, True, orig_nid, orig_eid)
        except Exception as e:
            print(e)

def start_dist_neg_dataloader(rank, ip_config, part_config, num_server,
        num_workers, orig_nid, groundtruth_g):
    import dgl
    import torch as th
    dgl.distributed.initialize(ip_config)
    gpb = None
    ...
    with tempfile.TemporaryDirectory() as test_dir:
        ...
        for i in range(num_server):
            p = ctx.Process(target=start_server, args=(
                i, ip_config, part_config, num_server > 1, num_workers+1))
            p.start()
            time.sleep(1)
        ...
        p = ctx.Process(target=start_dist_neg_dataloader, args=(
                0, ip_config, part_config, num_server, num_workers, orig_nid, g))
        p.start()
...
def test_dist_dataloader(num_server, num_workers, drop_last, reshuffle, num_groups):
    reset_envs()
    ...
    with tempfile.TemporaryDirectory() as test_dir:
        ...
        for i in range(num_server):
            p = ctx.Process(target=start_server, args=(
                i, ip_config, part_config, num_server > 1, num_workers+1, keep_alive))
            p.start()
            time.sleep(1)
            pserver_list.append(p)
        ...
        for trainer_id in range(num_trainers):
            for group_id in range(num_groups):
                p = ctx.Process(target=start_dist_dataloader, args=(
                    trainer_id, ip_config, part_config, num_server, drop_last, orig_nid, orig_eid, group_id))
                p.start()
...
def start_node_dataloader(rank, ip_config, part_config, num_server,
        num_workers, orig_nid, orig_eid, groundtruth_g):
    dgl.distributed.initialize(ip_config)
    gpb = None
    disable_shared_mem = num_server > 1
    if disable_shared_mem:
...
def start_edge_dataloader(rank, ip_config, part_config, num_server,
        num_workers, orig_nid, orig_eid, groundtruth_g):
    dgl.distributed.initialize(ip_config)
    gpb = None
    disable_shared_mem = num_server > 1
    ...
    with tempfile.TemporaryDirectory() as test_dir:
        ...
        if dataloader_type == 'node':
            p = ctx.Process(target=start_node_dataloader, args=(
                0, ip_config, part_config, num_server, num_workers, orig_nid, orig_eid, g))
            p.start()
            ptrainer_list.append(p)
...
def start_multiple_dataloaders(ip_config, part_config, graph_name, orig_g,
        num_dataloaders, dataloader_type):
    dgl.distributed.initialize(ip_config)
...
def test_multiple_dist_dataloaders(num_dataloaders, num_workers, dataloader_type):
    reset_envs()
    os.environ['DGL_DIST_MODE'] = 'distributed'
    os.environ['DGL_NUM_SAMPLER'] = str(num_workers)
    num_parts = 1
    ...
    with tempfile.TemporaryDirectory() as test_dir:
        ...
        for i in range(num_servers):
            p = ctx.Process(target=start_server,
                    args=(i, ip_config, part_config, num_servers > 1, num_workers + 1))
            p.start()
            time.sleep(1)
            p_servers.append(p)

        p_client = ctx.Process(target=start_multiple_dataloaders,
            args=(ip_config, part_config, graph_name, orig_g, num_dataloaders, dataloader_type))
        p_client.start()
