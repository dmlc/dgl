...
def check_compute_func(worker_id, graph_name, return_dict):
    try:
        ...
        # Test update all.
        g.update_all(fn.copy_u(src='feat', out='m'), fn.sum(msg='m', out='preprocess'))
        ...
        # Test pull
        g.pull(1, fn.copy_u(src='feat', out='m'), fn.sum(msg='m', out='tmp'))
        assert_almost_equal(F.asnumpy(data[1]), np.squeeze(F.asnumpy(g.nodes[1].data['preprocess'])))

        # Test send_and_recv
        in_edges = g.in_edges(v=2)
        g.send_and_recv(in_edges, fn.copy_u(src='feat', out='m'), fn.sum(msg='m', out='tmp'))
