...
def test_spmm(idtype, g, shp, msg, reducer):
    ...
    def f0(u, e):
        ...
        return F.reduce_sum(v)

    df0_du, df0_de = jax.grad(f0, argnums=(0, 1))(u, e)
    ...
    def f1(u, e):
        ...
        v1 = g.dstdata['v']
        return F.reduce_sum(v1)

    df1_du, df1_de = jax.grad(f1, argnums=(0, 1))(u, e)

    if msg != 'copy_rhs':
        if reducer in ['min', 'max']: # there might be some numerical errors
            rate = F.reduce_sum(F.abs(df1_du - df0_du)) /\
                   F.reduce_sum(F.abs(df0_du))
            assert F.as_scalar(rate) < 1e-2, rate
        else:
            assert F.allclose(df0_du, df1_du)

    if msg != 'copy_lhs':
        if reducer in ['min', 'max']: # there might be some numerical errors
            rate = F.reduce_sum(F.abs(df1_de - df0_de)) /\
                   F.reduce_sum(F.abs(df0_de))
            # print(F.reduce_sum(F.abs(df1_de) - df0_de), F.reduce_sum(F.abs(df0_de)))
            assert F.as_scalar(rate) < 1e-2, rate
        else:
            assert F.allclose(df0_de, df1_de)
...
def test_sddmm(g, shp, lhs_target, rhs_target, msg, idtype):
    ...
    def f0(feat_lhs, feat_rhs):
        ...
        return F.reduce_sum(g.edata['m'])

    def f1(feat_lhs, feat_rhs):
        e = gsddmm(g, msg, feat_lhs, feat_rhs, lhs_target=lhs_target, rhs_target=rhs_target)
        return F.reduce_sum(e)

    df0_dlhs, df0_drhs = jax.grad(f0, argnums=(0, 1))(feat_lhs, feat_rhs)
    df1_dlhs, df1_drhs = jax.grad(f1, argnums=(0, 1))(feat_lhs, feat_rhs)

    if msg != 'copy_rhs':
        assert F.allclose(df0_dlhs, df1_dlhs)
    if msg != 'copy_lhs':
        assert F.allclose(df0_drhs, df1_drhs)
    print('backward passed')
...
def test_edge_softmax(g, norm_by, shp, idtype):
    ...
    def f0(edata):
        score1 = edge_softmax(g, edata, norm_by=norm_by)
        return F.reduce_sum(score1)
    ...
    def f1(edata):
        ...
        if norm_by == 'dst':
            ...
            score2 = F.reshape(score2, (-1, *e2.shape[1:]))
        return F.reduce_sum(score2)

    df0 = jax.grad(f0)(edata)
    df1 = jax.grad(f1)(edata)

    assert F.allclose(df0, df1)
...
def test_segment_reduce(reducer):
    ...
    def f0(value):
        ...
        return F.reduce_sum(rst1)

    def f1(value):
        rst2 = segment_reduce(seglen, v2, reducer=reducer)
        return F.reduce_sum(rst2)

    df0 = jax.grad(f0)(value)
    df1 = jax.grad(f1)(value)
