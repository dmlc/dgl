...
def test_batch_unbatch1():
    ...
    s1, s2 = dgl.unbatch(bg)
    assert F.allclose(t1.ndata['h'], s1.ndata['h'])
    assert F.allclose(t1.edata['h'], s1.edata['h'])
    assert F.allclose(t2.ndata['h'], s2.ndata['h'])
    assert F.allclose(t2.edata['h'], s2.edata['h'])
