...
def random_feature(*shape):
    # dividing a small number generated by randn would cause overflows.
    return np.random.uniform(1, 1.5, shape).astype('float32')
...
def generate_feature(g, broadcast='none'):
    ...
    if broadcast == 'e':
        u = F.tensor(random_feature(nv, D1, D2, D3))
        e = F.tensor(random_feature(ne, D2, 1))
        v = F.tensor(random_feature(nv, D1, D2, D3))
    elif broadcast == 'u':
        u = F.tensor(random_feature(nv, D2, 1))
        e = F.tensor(random_feature(ne, D1, D2, D3))
        v = F.tensor(random_feature(nv, D1, D2, D3))
    elif broadcast == 'v':
        u = F.tensor(random_feature(nv, D1, D2, D3))
        e = F.tensor(random_feature(ne, D1, D2, D3))
        v = F.tensor(random_feature(nv, D2, 1))
    else:
        u = F.tensor(random_feature(nv, D1, D2, D3))
        e = F.tensor(random_feature(ne, D1, D2, D3))
        v = F.tensor(random_feature(nv, D1, D2, D3))
