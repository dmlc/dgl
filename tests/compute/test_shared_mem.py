...
def _assert_is_identical_heteroindex(g, g2):
    assert g.is_readonly() == g2.is_readonly()
    assert g.number_of_ntypes() == g2.number_of_ntypes()
    assert g.number_of_etypes() == g2.number_of_etypes()

    # check if two metagraphs are identical
    src, dst, _ = g.metagraph.edges(order='eid')
    src2, dst2, _ = g2.metagraph.edges(order='eid')
    src, dst, src2, dst2 = [x.tousertensor() for x in [src, dst, src2, dst2]]
    assert F.array_equal(src, src2)
    assert F.array_equal(dst, dst2)

    # check if node ID spaces and feature spaces are equal
    for ntype in range(g.number_of_ntypes()):
        assert g.number_of_nodes(ntype) == g2.number_of_nodes(ntype)

    # check if edge ID spaces and feature spaces are equal
    for etype in range(g.number_of_etypes()):
        src, dst, _ = g.edges(etype, order='eid')
        src2, dst2, _ = g2.edges(etype, order='eid')
        src, dst, src2, dst2 = [x.tousertensor() for x in [src, dst, src2, dst2]]
        assert F.array_equal(src, src2)
        assert F.array_equal(dst, dst2)
...
def sub_proc(hg_origin, name):
    hg_rebuild = dgl.heterograph_index.create_heterograph_from_shared_memory(name)
...
def test_copy_from_gpu():
    hg = create_test_graph(index_dtype="int32")._graph
