...
def test_joint_union():
    # homogeneous graph
    g1 = dgl.graph((F.tensor([0, 1, 3, 1]), F.tensor([1, 2, 0, 2])))
    g1.ndata['h'] = F.tensor([[0.], [1.], [2.], [1.]])
    g1.edata['h'] = F.tensor([[3.], [4.], [5.], [6.]])
    g2 = dgl.graph((F.tensor([3, 2]), F.tensor([1, 3])))
    ...
    assert F.array_equal(F.cat([v1, v2], dim=0),
                         v_ug)
    assert 'h' in ug.ndata
    assert 'h' in ug.edata
    assert F.array_equal(g1.ndata['h'], ug.ndata['h'])
    assert F.array_equal(F.cat([g1.edata['h'], g2.edata['h']], dim=0),
                         ug.edata['h'])
    ug.ndata['hh'] = F.tensor([[0.], [1.], [2.], [1.]])
    assert ('hh' in g1.ndata) is False
    assert ('hh' in g2.ndata) is False
    ug.edata['hh'] = F.tensor([[0.], [1.], [2.], [1.], [1.], [1.]])
    assert ('hh' in g1.edata) is False
    ...
    g2 = dgl.heterograph({
        ('user', 'follows', 'user'): (F.tensor([2]), F.tensor([1])),
        ('user', 'plays', 'game'): (F.tensor([0, 0, 1]), F.tensor([2, 1, 0]))
    })
    g2.edges['plays'].data['he'] = F.tensor([0, 0, 0])
    g3 = dgl.heterograph({
        ('user', 'follows', 'user'): (F.tensor([1]), F.tensor([2])),
    ...
    u3, v3 = g3.all_edges(order='eid', etype=('user', 'follows', 'user'))
    u_ug, v_ug = ug.all_edges(order='eid', etype=('user', 'follows', 'user'))
    assert F.array_equal(F.cat([u1, u2, u3], dim=0),
                         u_ug)
    assert F.array_equal(F.cat([v1, v2, v3], dim=0),
                         v_ug)
    assert len(ug.edges['follows'].data) == 0
    u1, v1 = g1.all_edges(order='eid', etype=('user', 'plays', 'game'))
    u2, v2 = g2.all_edges(order='eid', etype=('user', 'plays', 'game'))
    u3, v3 = g3.all_edges(order='eid', etype=('user', 'plays', 'game'))
    u_ug, v_ug = ug.all_edges(order='eid', etype=('user', 'plays', 'game'))
    assert F.array_equal(F.cat([u1, u2, u3], dim=0),
                         u_ug)
    assert F.array_equal(F.cat([v1, v2, v3], dim=0),
                         v_ug)
    assert 'he' in ug.edges['plays'].data
    assert F.array_equal(F.cat([g1.edges['plays'].data['he'],
                               g2.edges['plays'].data['he'],
                               g3.edges['plays'].data['he']], dim=0),
    ...
    assert len(ug.nodes['user'].data) == 0
    assert len(ug.edges['follows'].data) == 0
    assert len(ug.edges['plays'].data) == 0
    u1, v1 = g1.all_edges(order='eid', etype=('user', 'follows', 'user'))
    u2, v2 = g2.all_edges(order='eid', etype=('user', 'follows', 'user'))
    u3, v3 = g3.all_edges(order='eid', etype=('user', 'follows', 'user'))
    u_ug, v_ug = ug.all_edges(order='eid', etype=('user', 'follows', 'user'))
