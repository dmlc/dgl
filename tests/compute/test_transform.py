...
def test_add_reverse():
    ...
    def _check(g, new_g):
        ...
        src_AA = F.asnumpy(src_AA)
        dst_AA = F.asnumpy(dst_AA)
        src_BB = F.asnumpy(src_BB)
        dst_BB = F.asnumpy(dst_BB)
        assert np.array_equal(src_AA, np.array([0, 1, 1, 3]))
        assert np.array_equal(dst_AA, np.array([1, 3, 0, 1]))
        assert np.array_equal(src_BB, np.array([2, 4, 2, 3]))
        assert np.array_equal(dst_BB, np.array([2, 3, 2, 4]))
    ...
    src_AB = F.asnumpy(src_AB)
    dst_AB = F.asnumpy(dst_AB)
    assert np.array_equal(src_AB, np.array([0, 1]))
    assert np.array_equal(dst_AB, np.array([1, 2]))
...
def test_add_reverse_types():
    ...
    def _check(g, new_g):
        ...
        for etype in ['AA', 'AB', 'BB']:
            ...
            src = F.asnumpy(src)
            src_new = F.asnumpy(src_new)
            src_rev = F.asnumpy(src_rev)
            dst = F.asnumpy(dst)
            dst_new = F.asnumpy(dst_new)
            dst_rev = F.asnumpy(dst_rev)
            assert np.array_equal(src, dst_rev)
            assert np.array_equal(dst, src_rev)
            assert np.array_equal(src, src_new)
            assert np.array_equal(dst, dst_new)
