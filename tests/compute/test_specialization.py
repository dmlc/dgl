...
def test_v2v_update_all(idtype):
    def _test(fld):
        ...
        # update all
        v1 = g.ndata[fld]
        g.update_all(fn.copy_u(src=fld, out='m'), fn.sum(msg='m', out=fld), apply_func)
        v2 = g.ndata[fld]
        ...
        v1 = g.ndata[fld]
        g.update_all(fn.u_mul_e(src=fld, edge='e1', out='m'),
...
def test_v2v_snr(idtype):
    ...
    def _test(fld):
        ...
        # send and recv
        v1 = g.ndata[fld]
        g.send_and_recv((u, v), fn.copy_u(src=fld, out='m'),
                fn.sum(msg='m', out=fld), apply_func)
        v2 = g.ndata[fld]
        g.ndata.update({fld : v1})
        ...
        v1 = g.ndata[fld]
        g.send_and_recv((u, v), fn.u_mul_e(src=fld, edge='e1', out='m'),
                fn.sum(msg='m', out=fld), apply_func)
        v2 = g.ndata[fld]
        g.ndata.update({fld : v1})
...
def test_v2v_pull(idtype):
    ...
    def _test(fld):
        ...
        # send and recv
        v1 = g.ndata[fld]
        g.pull(nodes, fn.copy_u(src=fld, out='m'), fn.sum(msg='m', out=fld), apply_func)
        v2 = g.ndata[fld]
        g.ndata[fld] = v1
        ...
        # send and recv with edge weights
        v1 = g.ndata[fld]
        g.pull(nodes, fn.u_mul_e(src=fld, edge='e1', out='m'),
                fn.sum(msg='m', out=fld), apply_func)
        v2 = g.ndata[fld]
        g.ndata[fld] = v1
...
def test_update_all_multi_fallback(idtype):
    ...
    o3 = g.ndata.pop('o3')
    # v2v spmv
    g.update_all(fn.u_mul_e(src='h', edge='w1', out='m1'),
                 fn.sum(msg='m1', out='o1'),
                 _afunc)
    assert F.allclose(o1, g.ndata.pop('o1'))
    # v2v fallback to e2v
    g.update_all(fn.u_mul_e(src='h', edge='w2', out='m2'),
...
def test_pull_multi_fallback(idtype):
    ...
    def _pull_nodes(nodes):
        ...
        o3 = g.ndata.pop('o3')
        # v2v spmv
        g.pull(nodes, fn.u_mul_e(src='h', edge='w1', out='m1'),
                     fn.sum(msg='m1', out='o1'),
                     _afunc)
        assert F.allclose(o1, g.ndata.pop('o1'))
        # v2v fallback to e2v
        g.pull(nodes, fn.u_mul_e(src='h', edge='w2', out='m2'),
                     fn.sum(msg='m2', out='o2'),
                     _afunc)
        assert F.allclose(o2, g.ndata.pop('o2'))
