...
def test_node_removal():
    ...
    F.assert_array_equal(g.ndata['id'], F.tensor([0, 1, 2, 3, 7, 8, 9]))

    # add nodes
    g.add_nodes(3)
    assert g.number_of_nodes() == 10
    F.assert_array_equal(g.ndata['id'], F.tensor([0, 1, 2, 3, 7, 8, 9, 0, 0, 0]))

    # remove nodes
    g.remove_nodes(range(1, 4))
    assert g.number_of_nodes() == 7
    F.assert_array_equal(g.ndata['id'], F.tensor([0, 7, 8, 9, 0, 0, 0]))
...
def test_edge_removal():
    ...
    F.assert_array_equal(g.edata['id'], F.tensor(list(range(13)) + list(range(20, 25))))

    # add edges
    g.add_edge(3, 3)
    assert g.number_of_nodes() == 5
    assert g.number_of_edges() == 19
    F.assert_array_equal(g.edata['id'], F.tensor(list(range(13)) + list(range(20, 25)) + [0]))

    # remove edges
    g.remove_edges(range(2, 10))
    assert g.number_of_nodes() == 5
    assert g.number_of_edges() == 11
    F.assert_array_equal(g.edata['id'], F.tensor([0, 1, 10, 11, 12, 20, 21, 22, 23, 24, 0]))
...
def test_node_frame():
    ...
    F.assert_allclose(g.ndata['h'], F.zerocopy_from_numpy(new_data))
...
def test_edge_frame():
    ...
    F.assert_allclose(g.edata['h'], F.zerocopy_from_numpy(new_data))
