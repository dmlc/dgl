...
def test_simple_readout():
    ...
    assert th.allclose(dgl.mean_on(g1, 'nodes', 'x'), m1)
    assert th.allclose(dgl.mean_on(g1, 'nodes', 'x', 'w'), wm1)
    assert th.allclose(dgl.mean_on(g1, 'edges', 'x'), me1)

    g = dgl.batch([g1, g2])
    s = dgl.sum_on(g, 'nodes', 'x')
    m = dgl.mean_on(g, 'nodes', 'x')
    assert th.allclose(s, th.stack([s1, s2], 0))
    assert th.allclose(m, th.stack([m1, m2], 0))
    ws = dgl.sum_on(g, 'nodes', 'x', 'w')
    wm = dgl.mean_on(g, 'nodes', 'x', 'w')
