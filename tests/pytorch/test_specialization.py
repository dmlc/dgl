...
def generate_graph():
    ...
    g.add_edge(9, 0)
    g.set_n_repr({'f1' : F.randn((10,)).cuda(), 'f2' : F.randn((10, D)).cuda()})
    weights = F.randn((17,)).cuda()
...
def test_v2v_snr():
    ...
    def _test(fld):
        ...
        g.send_and_recv((u, v), fn.src_mul_edge(src=fld, edge='e1', out='m'),
                        fn.sum(msg='m', out=fld), apply_func)
        v2 = g.ndata[fld]
        g.set_n_repr({fld : v1.unsqueeze(1)})
...
def test_v2v_snr_multi_fn():
    ...
    g = generate_graph()
    g.set_n_repr({'v1' : F.zeros((10, D)).cuda(),
                  'v2' : F.zeros((10, D)).cuda(),
                  'v3' : F.zeros((10, D)).cuda()})
    fld = 'f2'
...
def test_update_all_multi_fallback():
    ...
    for i in range(1, 9):
        g.add_edge(0, i)
        g.add_edge(i, 9)
    # FIXME: we are not able to handle zero-deg correctly
    g.add_edge(9, 0)
    g.ndata['h'] = F.randn((10, D)).cuda()
    g.edata['w1'] = F.randn((17,)).cuda()
    g.edata['w2'] = F.randn((17, D)).cuda()
