...
std::shared_ptr<COO> COOFromOldDGLCOO(const aten::COOMatrix& dgl_coo) {
  ...
  auto data = DGLArrayToOptionalTorchTensor(dgl_coo.data);
  return std::make_shared<COO>(
      COO{dgl_coo.num_rows, dgl_coo.num_cols, row, col, data,
          dgl_coo.row_sorted, dgl_coo.col_sorted});
}
...
aten::COOMatrix COOToOldDGLCOO(const std::shared_ptr<COO>& coo) {
  ...
  auto data = OptionalTorchTensorToDGLArray(coo->data);
  return aten::COOMatrix(
      coo->num_rows, coo->num_cols, row, col, data, coo->row_sorted,
      coo->col_sorted);
}
...
std::shared_ptr<CSR> CSRFromOldDGLCSR(const aten::CSRMatrix& dgl_csr) {
  ...
  auto data = DGLArrayToOptionalTorchTensor(dgl_csr.data);
  return std::make_shared<CSR>(
      CSR{dgl_csr.num_rows, dgl_csr.num_cols, indptr, indices, data,
          dgl_csr.sorted});
}
...
aten::CSRMatrix CSRToOldDGLCSR(const std::shared_ptr<CSR>& csr) {
  ...
  auto data = OptionalTorchTensorToDGLArray(csr->data);
