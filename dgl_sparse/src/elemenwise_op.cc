...
/**
 * @brief Compute the intersection of two sparse matrices. Return the
 * intersection matrix and the indices of the intersection in the two matrices.
 */
std::tuple<std::shared_ptr<COO>, torch::Tensor, torch::Tensor>
SparseMatrixIntersection(
    c10::intrusive_ptr<SparseMatrix> lhs_mat,
    c10::intrusive_ptr<SparseMatrix> rhs_mat) {
  auto lhs_coo = lhs_mat->COOPtr();
  auto rhs_coo = rhs_mat->COOPtr();
  std::shared_ptr<COO> ret_coo;
  torch::Tensor lhs_indices, rhs_indices;
  std::tie(ret_coo, lhs_indices, rhs_indices) =
      COOIntersection(lhs_coo, rhs_coo);
...
variable_list SpSpMulAutoGrad::forward(
  ...
  std::tie(intersection, lhs_indices, rhs_indices) =
      SparseMatrixIntersection(lhs_mat, rhs_mat);
