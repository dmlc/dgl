...
class DGLDigitCapsuleLayer(nn.Module):
    def __init__(self, in_nodes_dim=8, in_nodes=1152, out_nodes=10, out_nodes_dim=16, device='cpu'):
        ...
        def cap_apply_edges(edge):
            return {'b_ij': edge['b_ij'] + (edge.dst['h'] * edge['u_hat']).mean(dim=1).sum(dim=1)}
    ...
    def construct_graph(self):
        ...
        input_nodes = list(range(self.in_nodes))
        output_nodes = list(range(self.in_nodes, self.in_nodes + self.out_nodes))
        u, v = [], []
