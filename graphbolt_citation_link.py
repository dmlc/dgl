...
def sampler_func(data):
    ...
    u = data[0]
    u = torch.cat([u, u.repeat_interleave(neg_ratio)])
    v = torch.stack(data[2:]).t().contiguous()
    # Cat pos and neg v.
    v = torch.cat([data[1], v.view(-1)])
    node_pairs = (u, v)
    
    seeds, compacted_pairs = gb.unique_and_compact_node_pairs(node_pairs)
    for hop in range(num_layers):
        sg = graph.sample_neighbors(seeds, torch.LongTensor(fanout[hop]))
...
def neg_sample_func(data):
    neg_pairs = graph.sample_negative_edges_uniform(edge_type=None, node_pairs=data, negative_ratio=neg_ratio)
    num_sample = neg_pairs[0].shape[0] // neg_ratio
...
negative_sampler = gb.SubgraphSampler(minibatch_sampler , neg_sample_func)
...
for epoch in range(1):
    model.train()
    for it, (input_features, compacted_pairs, blocks) in enumerate(
        dataloader
    ):
