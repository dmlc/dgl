 ...
 * then there will be multiple copies of the destination vertex in vids \return
 * a vector of 5 IdArrays for degree bucketing. The 5 arrays are: degrees:
 * degrees for each bucket nids: destination node ids nid_section: number of
 * nodes in each bucket (used to split nids) mids: message ids mid_section:
 * number of messages in each bucket (used to split mids)
 */
