  ...
  return AT_DISPATCH_INTEGRAL_TYPES(
      src_ids.scalar_type(), "unique_and_compact", ([&] {
        ...
        thrust::binary_search(
            exec_policy, sorted_unique_dst_ids.data_ptr<scalar_t>(),
            sorted_unique_dst_ids.data_ptr<scalar_t>() + unique_dst_ids.size(0),
            src_ids_ptr, src_ids_ptr + src_ids.size(0), is_dst.get());
