      ...
      0, num_index, group_size_, [&](int64_t begin, int64_t end) {
        ...
        if (!error_flag.load()) {
          ...
          for (int64_t i = begin; i < end; ++i) {
            ...
            //     int64_t feature_id = index_data[batch_start + i];  // Feature
            //     id. if (feature_id >= feature_dim_[0]) {
            //       error_flag.store(true);
            //       break;
            //     }
            //     //calculate offset of the feature.
            //     int64_t offset = feature_id * feature_size_ + prefix_len_;
            //     int64_t aligned_offset = offset & (long)~(kDiskAlignmentSize
            //     - 1);
            //     //put offset of the feature into array.
            //     residual[thread_id * group_size_ + group_id] =
            //         offset - aligned_offset;
            //     //If the tail of the feature extends into another block, read
            //     an additional block. int64_t read_size; if
            //     (residual[thread_id * group_size_ + group_id] + feature_size_
            //     >
            //         kDiskAlignmentSize) {
            //       read_size = aligned_length + kDiskAlignmentSize;
            //     } else {
            ...
            //     group. while (num_finish < batch_end) {
            //       struct io_uring_cqe *complete_queue;
            //       if (io_uring_wait_cqe(
            //               &io_uring_queue_[thread_id], &complete_queue) < 0)
            //               {
            ...
            //       }
            //       // Move the head pointer of completion queue.
            //       io_uring_cq_advance(&io_uring_queue_[thread_id],
            //       cqe_count); num_finish += cqe_count;
            //     }
            //     //copy results into result_buffer.
            //     for (int64_t batch_id = 0; batch_id < batch_end; batch_id++)
            //     {
            //       memcpy(
            //           result_buffer + feature_size_ * (batch_start +
            //           batch_id), read_buffer +
            //               ((aligned_length + kDiskAlignmentSize) *
            //               group_size_ *
            //                thread_id) +
