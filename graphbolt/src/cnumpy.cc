...
torch::Tensor OnDiskNpyArray::IndexSelectIOUringImpl(torch::Tensor index) {
  ...
  torch::parallel_for(0, num_thread_, 1, [&](int64_t begin, int64_t end) {
    ...
    auto &this_io_uring_queue = io_uring_queue_[thread_id];
    ...
    auto submit_fn = [&](int64_t submission_minimum_batch_size) {
      ...
      while (!read_queue.IsEmpty()) {
        const auto submitted = ::io_uring_submit(&this_io_uring_queue);
      ...
      AT_DISPATCH_INDEX_TYPES(
          index.scalar_type(), "IndexSelectIOUring", ([&] {
            ...
            for (int64_t i = begin; i < end; ++i) {
              ...
              struct io_uring_sqe *sqe = io_uring_get_sqe(&this_io_uring_queue);
              TORCH_CHECK(sqe);
      ...
      if (!error_flag.load(std::memory_order_relaxed)) {
        ...
        TORCH_CHECK(
            ::io_uring_wait_cqe_nr(
                &this_io_uring_queue, &cqe, num_submitted - num_completed) ==
            0);
        // Check the reads and abort on failure.
        int num_cqes_seen = 0;
        unsigned head;
        io_uring_for_each_cqe(&this_io_uring_queue, head, cqe) {
          const auto &req =
        ...
        io_uring_cq_advance(&this_io_uring_queue, num_cqes_seen);
